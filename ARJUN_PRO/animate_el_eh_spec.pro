...
pro animate_el_eh_spec,rate,units = units,range = range,trange = trange,sort = sort,var_label = var_label,ascending = ascending,wait = wait,ndata = ndata,picktimes = picktimes,inst = inst,numframes = numframes,_EXTRA = e
...
if not keyword_set(range) then range = [1.e5, 9.e7]
inst = 'get_' + inst
...
while i le n_elements(times)-1 and currFrame le nframes -1 do begin
	...
	get_el_eh_spec,time_string(times(i)-3),units = units,range = range,var_label = var_label,_EXTRA=e
