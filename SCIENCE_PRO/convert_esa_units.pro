...
;                                                 [# cm^(-3) km^(3) s^(-3)]
;                                'e2flux'      ;  energy flux squared?
;                                                 [eV^(+2) cm^(-2) s^(-1) sr^(-1) eV^(-1)]
;                                'e3flux'      ;  energy flux cubed?
;                                                 [eV^(+3) cm^(-2) s^(-1) sr^(-1) eV^(-1)]
;
;  EXAMPLES:    
;               [calling sequence]
...
;                                                                   [02/19/2016   v1.3.0]
;             6)  No longer calls convert_flux_units.pro [pointless call]
;                                                                   [02/19/2016   v1.3.1]
;             7)  For some reason I used the GEOM_FACTOR structure tag instead of
...
;    LAST MODIFIED:  04/14/2016   v1.3.2
...
;CASE STRUPCASE(data[0].UNITS_NAME) OF
  ...
  'E2FLUX' :  sfact = (dtc * (dt * gf)) / energy
  'E3FLUX' :  sfact = (dtc * (dt * gf)) / energy^2d0
  'DF'     :  sfact = (dtc * (dt * gf * energy^2d0 * 2d5/mass^2d0))
  ELSE: BEGIN
