...
;*****************************************************************************************
;
;  BATCH    :   example_load_Wind_mfi_swe_waves_3dp_spectra_0_batch.pro
...
;
;  CALLED BY:   
;               example_load_Wind_mfi_swe_waves_3dp_spectra_crib.pro
;
;  INCLUDES:
;               NA
;
...
IF (SIZE(sst_fout.SF_,/TYPE) NE 8) THEN nsf_ = 0 ELSE nsf_ = N_ELEMENTS(sst_fout.SF_)
...
IF (nsf_[0] EQ 0) THEN asf_ = 0 ELSE asf_ = sst_fout.SF_
...
IF (nsf_[0] GT 0) THEN add_scpot,asf_,scp_tpn[0]
...
IF (nsf_[0] GT 0) THEN add_scpot,asf_,scp_tpn[0],/LEAVE_ALONE
...
IF (nsf_[0] GT 0) THEN add_magf2,asf_,bgse_tpn[0]
...
IF (nsf_[0] GT 0) THEN add_magf2,asf_,bgse_tpn[0],/LEAVE_ALONE
...
IF (v3dp_onbrd_on[0]) THEN IF (nsf_[0] GT 0) THEN add_vsw2,asf_,vgse_tpn[0]
...
IF (nsf_[0] GT 0) THEN IF (v3dp_onbrd_on[0]) THEN add_vsw2,asf_,vgse_tpn[0],/LEAVE_ALONE ELSE add_vsw2,asf_,vgse_tpn[0]
...
IF (nsf_[0] GT 0) THEN add_vsw2,asf_,vgse_tpn[0],/LEAVE_ALONE
