...
;             2)  No real changes
;                                                                   [03/26/2008   v1.0.44]
;             3)  Rewrote and altered syntax to avoid bad Y-Ranges
;                                                                   [06/02/2009   v2.0.0]
;             4)  Fixed Y-Range estimates when < 4 pts in window
;                                                                   [06/03/2009   v2.0.1]
;             5)  Now calls my_box.pro
;                                                                   [06/03/2009   v2.0.2]
;             6)  Fixed issue when using keyword OVERPLOT
;                                                                   [09/04/2009   v2.0.3]
;             7)  Fixed typo when using keyword OVERPLOT
;                                                                   [09/16/2009   v2.0.4]
...
;                                                                   [11/13/2013   v2.2.1]
;            11)  Fixed an issue in YRANGE calculation when all data in window is < 0
;                                                                   [11/14/2013   v2.2.2]
;            12)  Fixed some bugs with finding XRANGE and when data input is > 2D
;                                                                   [02/11/2015   v2.3.0]
;
;   NOTES:      
...
;    LAST MODIFIED:  02/11/2015   v2.3.0
...
szy            = SIZE(y)
;;  LBW III  02/11/2015   v2.3.0
szdy           = SIZE(dy)
szn_test       = (SIZE(y,/N_DIMENSIONS) EQ SIZE(dy,/N_DIMENSIONS))
IF (szy[0] LT 1) THEN d1 = 1 ELSE d1 = szy[1]
IF (szy[0] LT 2) THEN d2 = 1 ELSE d2 = szy[2]
;;  LBW III  02/11/2015   v2.3.0
IF (szy[0] LT 3) THEN d3 = 1 ELSE d3 = szy[3]
...
;  IF (count EQ 0L) THEN w = N_ELEMENTS(x)/2
  ...
  x  = x[mm[0]:mm[1]]
;;  LBW III  02/11/2015   v2.3.0
  IF (d3[0] GT 1) THEN BEGIN
    ;;  Y is a 3D array
    y  = y[mm[0]:mm[1],*,*]
    IF (KEYWORD_SET(dy) AND szn_test[0]) THEN dy = dy[mm[0]:mm[1],*,*]
  ENDIF ELSE BEGIN
    ;;  Only a 2D array
    y  = y[mm[0]:mm[1],*]
    IF (KEYWORD_SET(dy) AND szn_test[0]) THEN dy = dy[mm[0]:mm[1],*]
...
;  IF (yrange[0] LT 0.) THEN yrange *= 1.015 ELSE yrange *= [1/1.01,1.015]
  ;;  LBW III 11/14/2013   v2.2.2
  IF (yrange[0] EQ yrange[1]) THEN BEGIN
    ;;  Bad data --> use default
    yrange = [0.,1.]
  ENDIF ELSE BEGIN
    ;;  Good data
    IF (yrange[0] LT 0. AND yrange[1] GE 0.) THEN BEGIN
      ;;  Data is positive and negative
      yrange *= 1.015
    ENDIF ELSE BEGIN
      IF (yrange[0] LT 0. AND yrange[1] LT 0.) THEN BEGIN
...
;;  LBW III  02/11/2015   v2.3.0
  IF (d3[0] GT 1) THEN BEGIN
    ;;  Y is a 3D array
    yt0 = REFORM(y[*,n,*])
    ;;  Avg. over 3rd dimensions
    yt  = TOTAL(yt0,2L,/NAN)/TOTAL(FINITE(yt0),2L,/NAN)
  ENDIF ELSE BEGIN
    ;;  Only a 2D array
    yt  = y[*,n]
  ENDELSE
...
;  IF (NOT KEYWORD_SET(noerrorbars) AND N_ELEMENTS(dy) NE 0) THEN BEGIN
  IF (NOT KEYWORD_SET(noerrorbars) AND ((N_ELEMENTS(dy) NE 0) AND szn_test[0])) THEN BEGIN
    tempc    = !P.COLOR
    !P.COLOR = c
;;  LBW III  02/11/2015   v2.3.0
;    upper    = yt + dy[*,n]
;    lower    = yt - dy[*,n]
    IF (d3[0] GT 1) THEN BEGIN
      ...
      dy0 = REFORM(dy[*,n,*])
      ;;  Avg. over 3rd dimensions
      dy1 = TOTAL(dy0,2L,/NAN)/TOTAL(FINITE(dy0),2L,/NAN)
    ENDIF ELSE BEGIN
      ;;  Only a 2D array
      dy1 = dy[*,n]
    ENDELSE
