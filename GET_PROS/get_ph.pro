...
;  EXAMPLES:    
;               [calling sequence]
;               ph = get_ph([t] [,add] [,/TIMES] [,INDEX=idx] [,/ADVANCE])
;
;               ;;  Example
;               t  = '1995-01-01/00:00:00'  ;;  Define a timestamp of interest
;               td = time_double(t)         ;;  Convert to Unix
;               ph = get_ph(td[0])          ;;  Get IDL structure of distribution
;
;  KEYWORDS:    
...
IF KEYWORD_SET(tms) THEN BEGIN
  ...
  PRINT, ok[0] + 1, '  Pesa high time samples'
...
CASE mapcode[0] OF
  ...
  ELSE: BEGIN 
    ...
    MESSAGE,'Unknown ph data map ('+mapstr[0]+') requested or bad packet',/INFORMATIONAL,/CONTINUE
    RETURN,{DATA_NAME:'null',VALID:0}
  END
ENDCASE
;;----------------------------------------------------------------------------------------
