...
function get_elc,t,add, times=tms, index=idx  ;, advance = adv
@wind_com.pro

dat = { elc_struct, $
   PROJECT_NAME:   'Wind 3D Plasma', $
   DATA_NAME:      'Eesa Low Cuts', $
   UNITS_NAME:     'Counts', $
   ...
   DATA      :     fltarr(15, 32), $
   energy    :     fltarr(15, 32), $
   denergy   :     fltarr(15, 32), $
   phi: fltarr(15, 32), $
   dphi: fltarr(15, 32), $
   theta: fltarr(15, 32), $
   dtheta: fltarr(15, 32), $
   bins      :     replicate(1b,15,32), $
   dt        :     fltarr(15,32), $
   gf        :     fltarr(15,32), $
   bkgrate   :     fltarr(15,32), $
   deadtime  :     fltarr(15,32), $
   dvolume   :     fltarr(15,32), $
   ddata     :     replicate(!values.f_nan,15,32), $
...
;   num = call_external(wind_lib,'elc_to_idl')
   num = 10000
   options(0) = num
   times = dblarr(num)
   ok = call_external(wind_lib,'elc_to_idl',options,times)
   print,ok+1,'  Eesa low cut time samples'
...
oldtime=dat.time
repeat begin
	ok = call_external(wind_lib,'elc_to_idl',options,time,dat)
