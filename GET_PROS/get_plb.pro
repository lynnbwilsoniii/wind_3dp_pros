...
;                 function (VDF) data pertinent to a single PESA Low Burst 3D data sample
...
;                                                                   [07/25/2011   v1.2.0]
;             4)  Updated Man. page and rewrote routine
;                                                                   [05/14/2021   v1.2.1]
;             5)  Changed how routine handles large time ranges for determining the
;                   number of VDFs
;                                                                   [02/28/2024   v1.2.2]
;             6)  Fixed a bug if pl8x8_to_idl.c returned an empty array
;                                                                   [03/01/2024   v1.2.3]
;
;   NOTES:      
...
;    LAST MODIFIED:  03/01/2024   v1.2.3
...
IF KEYWORD_SET(tms) THEN BEGIN
  ...
  IF (verm[0] GE 64) THEN num = CALL_EXTERNAL(wind_lib,'pl8x8_to_idl') > 1L ELSE num = 10000L
  options[0]     = num[0]
  times          = DBLARR(num)
  ok             = CALL_EXTERNAL(wind_lib,'pl8x8_to_idl',options,times)
  IF (SIZE(ok,/TYPE) EQ 0) THEN RETURN,0d0
  MESSAGE,STRTRIM(STRING(ok[0] + 1L,FORMAT='(I)'),2L)+'  Pesa low burst time samples',/CONTINUE,/INFORMATIONAL
