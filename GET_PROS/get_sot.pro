...
;*****************************************************************************************
;
;  FUNCTION :   get_sot.pro
;  PURPOSE  :   This routine returns a 3D structure containing all velocity distribution
;                 function (VDF) data pertinent to a single SST Open+Thick 3D data sample
...
;  EXAMPLES:    
;               [calling sequence]
;               sot = get_sot([t] [,add] [,/TIMES] [,INDEX=idx] [,/CHANMODE])
;
;               ;;  Example
;               t   = '1995-01-01/00:00:00'  ;;  Define a timestamp of interest
;               td  = time_double(t)         ;;  Convert to Unix
;               sot = get_sot(td[0])         ;;  Get IDL structure of distribution
;
;  KEYWORDS:    
...
;                                                                   [02/28/2024   v1.1.0]
;             3)  Fixed a bug if sot_to_idl.c returned an empty array
;                                                                   [03/01/2024   v1.1.1]
;
;   NOTES:      
...
dat = {sot_struct2,                                 $
        ...
        DATA             :     FLTARR( 9,16),       $           ;;  " " data values [UNITS_NAME]
        ENERGY           :     FLTARR( 9,16),       $           ;;  " " energy bin mid-point values [eV] = (E_upp + E_low)/2
        DENERGY          :     FLTARR( 9,16),       $           ;;  " " energy bin widths/sizes [eV] = (E_upp - E_low)
        PHI              :     FLTARR( 9,16),       $           ;;  " " azimuthal mid-point GSE angles [deg] for each data point = (phi_upp + phi_low)/2
        DPHI             :     FLTARR( 9,16),       $           ;;  " " azimuthal bin widths/sizes [deg] = (phi_upp - phi_low)
        THETA            :     FLTARR( 9,16),       $           ;;  " " poloidal mid-point GSE angles [deg] from -90 to +90 degrees = (theta_upp + theta_low)/2
        DTHETA           :     FLTARR( 9,16),       $           ;;  " " poloidal bin widths/sizes [deg] = (theta_upp - theta_low)
        BINS             :     REPLICATE(1b, 9,16), $           ;;  " " byte values indicating whether to use or ignore any given energy-angle bin (used by other routines)
        DT               :     FLTARR( 9,16),       $           ;;  " " anode accumulation times [s] for any given energy-angle bin
        GF               :     FLTARR( 9,16),       $           ;;  " " relative geometric factors per energy-angle bin used in unit conversion routines to correct GEOMFACTOR
        BKGRATE          :     FLTARR( 9,16),       $           ;;  " " background values [UNITS_NAME] --> User is expected to determine these
        DEADTIME         :     FLTARR( 9,16),       $           ;;  " " deadtimes [s], i.e., times when instrument not taking or unable to take data [e.g., Meeks and Siegel, 2008]
        DVOLUME          :     FLTARR( 9,16),       $           ;;  " " defining the differential volume for each energy-angle bin [sr]
        DDATA            :     REPLICATE(f, 9,16),  $           ;;  " " uncertainties for each data point [UNITS_NAME] --> User is expected to determine these
...
IF KEYWORD_SET(tms) THEN BEGIN
  ...
  IF (verm[0] GE 64) THEN num = CALL_EXTERNAL(wind_lib,'sot_to_idl') > 1L ELSE num = 100000L
  options[0]     = num[0]
  times          = DBLARR(num)
  ok             = CALL_EXTERNAL(wind_lib,'sot_to_idl',options,times)
  IF (SIZE(ok,/TYPE) EQ 0) THEN RETURN,0d0
  MESSAGE,STRTRIM(STRING(ok[0] + 1L,FORMAT='(I)'),2L)+'  SST Open+Thick time samples',/CONTINUE,/INFORMATIONAL
...
;;----------------------------------------------------------------------------------------
REPEAT BEGIN
  ok             = CALL_EXTERNAL(wind_lib,'sot_to_idl',options,time[0],dat)
