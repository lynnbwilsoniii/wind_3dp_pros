...
;*****************************************************************************************
;
;  FUNCTION :   get_ehb.pro
;  PURPOSE  :   This routine returns a 3D structure containing all velocity distribution
;                 function (VDF) data pertinent to a single EESA High Burst 3D data sample
...
;  EXAMPLES:    
;               [calling sequence]
;               ehb = get_ehb([t] [,add] [,/TIMES] [,INDEX=idx] [,/ADVANCE])
;
;               ;;  Example
;               t   = '1995-01-01/00:00:00'  ;;  Define a timestamp of interest
;               td  = time_double(t)         ;;  Convert to Unix
;               ehb = get_ehb(td[0])         ;;  Get IDL structure of distribution
;
;  KEYWORDS:    
...
;                   number of VDFs
;                                                                   [02/28/2024   v1.1.1]
;             4)  Fixed a bug if ehb_to_idl.c returned an empty array
;                                                                   [03/01/2024   v1.1.2]
;
;   NOTES:      
...
FUNCTION get_ehb,t,add,TIMES=tms,INDEX=idx,ADVANCE=adv
...
IF KEYWORD_SET(tms) THEN BEGIN
  ...
  IF (verm[0] GE 64) THEN num = CALL_EXTERNAL(wind_lib,'ehb_to_idl') > 1L ELSE num = 10000L
  options[0]     = num[0]
  times          = DBLARR(num)
  ok             = CALL_EXTERNAL(wind_lib,'ehb_to_idl',options,times)
  IF (SIZE(ok,/TYPE) EQ 0) THEN RETURN,0d0
  MESSAGE,STRTRIM(STRING(ok[0] + 1L,FORMAT='(I)'),2L)+'  Eesa high burst time samples',/CONTINUE,/INFORMATIONAL
...
;;----------------------------------------------------------------------------------------
REPEAT BEGIN
  ok             = CALL_EXTERNAL(wind_lib,'ehb_to_idl',options,time,dat)
...
;;----------------------------------------------------------------------------------------
retdat.GEOMFACTOR = retdat.GEOMFACTOR[0] * 0.8
;;  Define detector deadtime  [+LBW 05/14/2021   v1.1.0]
retdat.DEADTIME  += 1e-7      ;;  Put a lower bound on it so the values aren't zero, which is unphysical
;;----------------------------------------------------------------------------------------
;;  Return to user
;;----------------------------------------------------------------------------------------
