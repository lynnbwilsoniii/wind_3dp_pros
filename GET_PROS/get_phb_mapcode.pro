...
;  PURPOSE  :   Program returns the current mapcode for Pesa High Burst relevant to the
...
;  EXAMPLES:    
;               [calling sequence]
;               mapphb = get_phb_mapcode(t [,/ADVANCE] [,INDEX=idx] [,OPTIONS=options] [,/PRESET])
;
;               ;;  Example
...
IF NOT KEYWORD_SET(preset) THEN BEGIN
  ...
  ;;--------------------------------------------------------------------------------------
  IF (N_ELEMENTS(options) LT 3) THEN options = [15,0,0]
...
IF (ok[0] EQ 0) THEN BEGIN 
  MESSAGE,'get_phb_mapcode:  failed to get packet type',/INFORMATIONAL,/CONTINUE
  mapcode        = 0
ENDIF
;;----------------------------------------------------------------------------------------
;;  Return to user
