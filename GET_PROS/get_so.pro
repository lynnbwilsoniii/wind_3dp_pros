...
;                 function (VDF) data pertinent to a single SST Open 3D data sample or
...
;  EXAMPLES:    
;               [calling sequence]
;               so = get_so([t] [,add] [,/TIMES] [,INDEX=idx] [,/CHANMODE])
;
;               ;;  Example
;               t  = '1995-01-01/00:00:00'  ;;  Define a timestamp of interest
;               td = time_double(t)         ;;  Convert to Unix
;               so = get_so(td[0])          ;;  Get IDL structure of distribution
;
;  KEYWORDS:    
...
;                                                                   [02/28/2024   v1.1.0]
;             3)  Fixed a bug if sst_open_to_idl.c returned an empty array
;                                                                   [03/01/2024   v1.1.1]
;
;   NOTES:      
...
IF KEYWORD_SET(tms) THEN BEGIN
  ...
  IF (verm[0] GE 64) THEN num = CALL_EXTERNAL(wind_lib,'sst_open_to_idl') > 1L ELSE num = 100000L
  options[0]     = num[0]
  times          = DBLARR(num)
  ok             = CALL_EXTERNAL(wind_lib,'sst_open_to_idl',options,times)
  IF (SIZE(ok,/TYPE) EQ 0) THEN RETURN,0d0
  MESSAGE,STRTRIM(STRING(ok[0] + 1L,FORMAT='(I)'),2L)+'  SST Open time samples',/CONTINUE,/INFORMATIONAL
