...
tdat  = tdats.DATA
times = tdats.TIMES
mynn  = N_ELEMENTS(tdat)
;*****************************************************************************************
; -Get B-field data for each structure
;*****************************************************************************************
add_magf2,tdat,mag_name
;*****************************************************************************************
; -Calc 3D moments from data structures
;*****************************************************************************************
FOR n=0L,mynn-1L DO BEGIN
    dat = tdat[n]
    IF dat.valid THEN BEGIN
      IF ions THEN BEGIN
        todat = total(dat.data,2)
        mx    = MAX(todat,mxb)
        IF KEYWORD_SET(protons) THEN er = [mxb-3L,13L] ELSE er = [0L,mxb-4L]
