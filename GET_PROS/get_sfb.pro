...
;*****************************************************************************************
;
;  FUNCTION :   get_sfb.pro
;  PURPOSE  :   This routine returns a 3D structure containing all velocity distribution
;                 function (VDF) data pertinent to a single SST Foil Burst 3D data sample
...
;                                                                   [02/28/2024   v1.1.0]
;             3)  Fixed a bug if sfb_to_idl.c returned an empty array
;                                                                   [03/01/2024   v1.1.1]
;
;   NOTES:      
...
IF KEYWORD_SET(tms) THEN BEGIN
  ...
  IF (verm[0] GE 64) THEN num = CALL_EXTERNAL(wind_lib,'sfb_to_idl') > 1L ELSE num = 100000L
  options[0]     = num[0]
  times          = DBLARR(num)
  ok             = CALL_EXTERNAL(wind_lib,'sfb_to_idl',options,times)
  IF (SIZE(ok,/TYPE) EQ 0) THEN RETURN,0d0
  MESSAGE,STRTRIM(STRING(ok[0] + 1L,FORMAT='(I)'),2L)+'  SST Foil Burst time samples',/CONTINUE,/INFORMATIONAL
