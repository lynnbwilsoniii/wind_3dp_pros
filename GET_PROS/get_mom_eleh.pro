...
;VERSION %I%
;-
pro get_mom_eleh,name=name,trange=trange, $
  sc_pot=sc_pot,  $
  dens_name =dens_name,  $
  elbkg = elbkg, $
  ehbkg = ehbkg, $
...
ehtime=get_eh(/times)
ehindex = round( interp(time,lindgen(max)+start_index,ehtime) )
...
;    eh = get_eh(index=ehindex[n])
    eh = get_eh(dat.time-1)
    if keyword_set(elbkg) then  dat=sub3d(dat,elbkg)
    if keyword_set(ehbkg) then  eh =sub3d(eh,ehbkg)
...
;       ds[n] = mom3d_eleh(dat,dens[n],format=str_format)
;     endif else    ds[n] = mom3d(dat,erange=er,format=str_format)
      ds[n] = mom_eleh(dat,eh,format=str_format)   ;,sc_pot=sc_pot)
;      ds[n] = mom_eleh(dat,eh,format=str_format)
    endif
    print,n,max,'  ',time_string(dat.time),dat.time-eh.time
endfor
...
if not keyword_set(name) then name = 'elhm
