...
;                 function (VDF) data pertinent to a single EESA High 3D data sample or
...
;  EXAMPLES:    
;               [calling sequence]
;               eh = get_eh([t] [,add] [,/TIMES] [,INDEX=idx] [,/ADVANCE])
;
;               ;;  Example
;               t  = '1995-01-01/00:00:00'  ;;  Define a timestamp of interest
;               td = time_double(t)         ;;  Convert to Unix
;               eh = get_eh(td[0])          ;;  Get IDL structure of distribution
;
;  KEYWORDS:    
...
;                   number of VDFs
;                                                                   [02/28/2024   v1.1.1]
;             4)  Fixed a bug if e3dunk_to_idl.c returned an empty array
;                                                                   [03/01/2024   v1.1.2]
;
;   NOTES:      
...
IF KEYWORD_SET(tms) THEN BEGIN
  ...
  IF (verm[0] GE 64) THEN num = CALL_EXTERNAL(wind_lib,'e3dunk_to_idl') > 1L ELSE num = 10000L
  options[0]     = num[0]
  times          = DBLARR(num)
  ok             = CALL_EXTERNAL(wind_lib,'e3dunk_to_idl',options,times)
  IF (SIZE(ok,/TYPE) EQ 0) THEN RETURN,0d0
  MESSAGE,STRTRIM(STRING(ok[0] + 1L,FORMAT='(I)'),2L)+'  Eesa high time samples',/CONTINUE,/INFORMATIONAL
