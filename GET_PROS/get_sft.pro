...
;*****************************************************************************************
;
;  FUNCTION :   get_sft.pro
;  PURPOSE  :   This routine returns a 3D structure containing all velocity distribution
;                 function (VDF) data pertinent to a single SST Foil+Thick 3D data sample
...
;  EXAMPLES:    
;               [calling sequence]
;               sft = get_sft([t] [,add] [,/TIMES] [,INDEX=idx] [,/CHANMODE])
;
;               ;;  Example
;               t   = '1995-01-01/00:00:00'  ;;  Define a timestamp of interest
;               td  = time_double(t)         ;;  Convert to Unix
;               sft = get_sft(td[0])         ;;  Get IDL structure of distribution
;
;  KEYWORDS:    
...
;               CHANMODE  :  If set, load only channels that are read out for the
...
;                                                                   [02/28/2024   v1.1.0]
;             3)  Fixed a bug if sft_to_idl.c returned an empty array
;                                                                   [03/01/2024   v1.1.1]
;
;   NOTES:      
...
;    LAST MODIFIED:  03/01/2024   v1.1.1
...
dat = { sft_struct2,                                $
        ...
        NBINS            :     16,                  $           ;;  " " integer defining the number of solid-angle bins
        NENERGY          :      7,                  $           ;;  " " integer defining the number of energy bins
        DETECTOR         :     INTARR(16),          $           ;;  Not sure but best guess is it's a logic array to select different look directions
        DATA             :     FLTARR( 7,16),       $           ;;  " " data values [UNITS_NAME]
        ENERGY           :     FLTARR( 7,16),       $           ;;  " " energy bin mid-point values [eV] = (E_upp + E_low)/2
        DENERGY          :     FLTARR( 7,16),       $           ;;  " " energy bin widths/sizes [eV] = (E_upp - E_low)
        PHI              :     FLTARR( 7,16),       $           ;;  " " azimuthal mid-point GSE angles [deg] for each data point = (phi_upp + phi_low)/2
        DPHI             :     FLTARR( 7,16),       $           ;;  " " azimuthal bin widths/sizes [deg] = (phi_upp - phi_low)
        THETA            :     FLTARR( 7,16),       $           ;;  " " poloidal mid-point GSE angles [deg] from -90 to +90 degrees = (theta_upp + theta_low)/2
        DTHETA           :     FLTARR( 7,16),       $           ;;  " " poloidal bin widths/sizes [deg] = (theta_upp - theta_low)
        BINS             :     REPLICATE(1b, 7,16), $           ;;  " " byte values indicating whether to use or ignore any given energy-angle bin (used by other routines)
        DT               :     FLTARR( 7,16),       $           ;;  " " anode accumulation times [s] for any given energy-angle bin
        GF               :     FLTARR( 7,16),       $           ;;  " " relative geometric factors per energy-angle bin used in unit conversion routines to correct GEOMFACTOR
        BKGRATE          :     FLTARR( 7,16),       $           ;;  " " background values [UNITS_NAME] --> User is expected to determine these
        DEADTIME         :     FLTARR( 7,16),       $           ;;  " " deadtimes [s], i.e., times when instrument not taking or unable to take data [e.g., Meeks and Siegel, 2008]
        DVOLUME          :     FLTARR( 7,16),       $           ;;  " " defining the differential volume for each energy-angle bin [sr]
        DDATA            :     REPLICATE(f, 7,16),  $           ;;  " " uncertainties for each data point [UNITS_NAME] --> User is expected to determine these
        ...
        DOMEGA           :     FLTARR(16),          $           ;;  Array of solid-angles [sr] spanned by each solid-angle bin
        FEFF             :     FLTARR( 7,16)        $           ;;  Array of detector efficiencies for any given energy-angle bin
}
;;----------------------------------------------------------------------------------------
;;  Check input
;;----------------------------------------------------------------------------------------
...
IF KEYWORD_SET(tms) THEN BEGIN
  ...
  IF (verm[0] GE 64) THEN num = CALL_EXTERNAL(wind_lib,'sft_to_idl') > 1L ELSE num = 100000L
  options[0]     = num[0]
  times          = DBLARR(num)
  ok             = CALL_EXTERNAL(wind_lib,'sft_to_idl',options,times)
  IF (SIZE(ok,/TYPE) EQ 0) THEN RETURN,0d0
  MESSAGE,STRTRIM(STRING(ok[0] + 1L,FORMAT='(I)'),2L)+'  SST Foil+Thick time samples',/CONTINUE,/INFORMATIONAL
