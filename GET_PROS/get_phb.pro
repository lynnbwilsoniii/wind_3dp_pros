...
;                 function (VDF) data pertinent to a single PESA High Burst 3D data sample
...
;    LAST MODIFIED:  02/28/2024   v1.1.2
...
IF KEYWORD_SET(tms) THEN BEGIN
  ...
  MESSAGE,STRTRIM(STRING(ok[0] + 1L,FORMAT='(I)'),2L)+'  PESA High Burst time samples',/CONTINUE,/INFORMATIONAL
...
CASE mapcode[0] OF
  ...
  ELSE: BEGIN 
    ...
    MESSAGE,'Unknown PESA High Burst data map ('+mapstr[0]+') requested or bad packet',/INFORMATIONAL,/CONTINUE
    RETURN,{DATA_NAME:'null',VALID:0}
  END
ENDCASE
;;----------------------------------------------------------------------------------------
