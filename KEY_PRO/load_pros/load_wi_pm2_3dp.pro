...
if not keyword_set(masterfile) then $
   masterfile = '/disks/boreas/disk2/davin/data/wi/3dp/pm/*.cdf

cdfnames = ['Np',  'Vp', 'Tp','Tp_rat','RVVp','magf']
cdfnames = ['Np',  'Vp', 'Tp','Tp_rat']
...
if data_type(prefix) eq 7 then px=prefix else px = ''

vxlab = ['Vx','Vy','Vz']

store_data,px+'Np',data={x:d.time,y:d.NP},min=-1e30,  $
  dlim={ytitle:'N!dP!n'}
store_data,px+'Vp',data={x:d.time,y:dimen_shift(d.VP,1)},min=-1e30,$
  dlim={ytitle:'V!dP!n',labels:vxlab}
...
store_data,px+'Tp',data={x:d.time,y:d.TP},min=-1e30
store_data,px+'Tp_RAT',data={x:d.time,y:d.TP_RAT},min=-1e30
