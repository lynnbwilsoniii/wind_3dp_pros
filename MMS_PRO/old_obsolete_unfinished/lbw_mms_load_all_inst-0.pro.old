...
;    LAST MODIFIED:  06/29/2018   v1.0.0
...
;;  Load quaternions
mms_load_mec,TRANGE=trs,PROBE=sc,VARFORMAT='*_quat_*'
;;----------------------------------------------------------------------------------------
;;----------------------------------------------------------------------------------------
;;----------------------------------------------------------------------------------------
...
mms_load_fgm,TRANGE=tran,PROBES=sc,DATA_RATE=ssfb_str[0],LEVEL='l2',$
             /GET_SUPPORT_DATA,/LATEST_VERSION,SPDF=spdf
...
FOR j=0L, n_fgm_tp[0] - 1L DO BEGIN
  ...
  get_data,all_fgm_gse_tp[j],DATA=t_bgsm_vm,DLIMIT=dlim_bgsm,LIMIT=lim_bgsm
  ;;  Define parameters
  ...
  fgm_mag_tpnout = scpref[0]+'fgm_'+ssfb_str[j]+'_l2_'+coord_mag[0]
...
mms_load_scm,TRANGE=tran,PROBES=sc,DATA_RATE=ssfb_str[3],DATATYPE=d_types,$
             LEVEL='l2',/GET_SUPPORT_DATA,/LATEST_VERSION,SPDF=spdf
...
FOR j=0L, n_t[0] - 1L DO BEGIN
  mms_load_edp,TRANGE=tran,PROBES=sc,LEVEL='l2',DATATYPE=d_types[j],$
               DATA_RATE=ssfb_str[2],/GET_SUPPORT_DATA,/LATEST_VERSION,SPDF=spdf
  mms_load_edp,TRANGE=tran,PROBES=sc,LEVEL='l2',DATATYPE=d_types[j],$
               DATA_RATE=ssfb_str[3],/GET_SUPPORT_DATA,/LATEST_VERSION,SPDF=spdf
ENDFOR
;;  Check RM_EDP_SPRT
...
ENDFOR
;;----------------------------------------------------------------------------------------
;;  Rotate Bo from GSE to DMPA
;;----------------------------------------------------------------------------------------
;;  Define input coordinate (GSE) and output coordinate (DBCS) basis
in__coord      = coord_gse[0]
out_coord      = coord_dmpa[0]
;;  Define input and output TPLOT handles
in__names      = [fgm_s__gse_tpn[0],fgm_l__gse_tpn[0],fgm_f__gse_tpn[0],fgm_b__gse_tpn[0]]
out_names      = scpref[0]+'fgm_'+ssfb_str+'_l2_'+out_coord[0]
FOR j=0L, N_ELEMENTS(in__names) - 1L DO BEGIN
...
ENDFOR
;;----------------------------------------------------------------------------------------
;;  Rotate Vbulk from GSE to DMPA
;;----------------------------------------------------------------------------------------
;;  Define input coordinate (GSE) and output coordinate (DMPA) basis
in__coord      = coord_gse[0]
out_coord      = coord_dmpa[0]
;;  Rotate Vbulk from GSE to DMPA
