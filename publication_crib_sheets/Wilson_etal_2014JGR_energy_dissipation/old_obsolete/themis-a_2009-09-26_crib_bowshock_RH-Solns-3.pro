...
;;  Bo
get_data,fgmnm[1],DATA=th_magf,DLIM=dlim,LIM=lim
...
peib_tn   = 'peib_'
peeb_tn   = 'peeb_'
denname   = pref[0]+peib_tn[0]+'density'
ti_name   = pref[0]+peib_tn[0]+'avgtemp'
te_name   = pref[0]+peeb_tn[0]+'avgtemp'
velname   = pref[0]+peib_tn[0]+'velocity_'+coord[0]
names     = [sm_fgmnm[0],denname[0],ti_name[0],te_name[0],velname[0]]
  tplot,names,TRANGE=tr_jj
;;----------------------------------------------------------------------------------------
;;----------------------------------------------------------------------------------------
;;  => Shock Param Summary
;;----------------------------------------------------------------------------------------
...
;; Upstream/Downstream time ranges
;tr_up          = time_double(tdate[0]+'/'+['15:54:25','15:57:30'])
tr_up          = time_double(tdate[0]+'/'+['15:55:05','15:57:30'])
tr_dn          = time_double(tdate[0]+'/'+['15:49:30','15:51:55'])
...
peib_tn        = 'peib_'
peeb_tn        = 'peeb_'
vsw_name       = tnames(pref[0]+peib_tn[0]+'velocity_'+coord_in[0])
den_name       = tnames(pref[0]+peib_tn[0]+'density')
Ti__name       = tnames(pref[0]+peib_tn[0]+'avgtemp')
Te__name       = tnames(pref[0]+peeb_tn[0]+'avgtemp')
fglgse         = tnames(sm_fgmnm[0])

names          = [fglgse[0],den_name[0],Ti__name[0],Te__name[0],vsw_name[0]]
  tplot,names,TRANGE=tr_jj
  time_bar,tr_up,VARNAME=names,COLOR=250L
  time_bar,tr_dn,VARNAME=names,COLOR= 50L


prefnm         = 'THA_sm-fgl-30pts_Ni-Ti-Te-Vi'
suffnm         = '1st-time-range'
  tplot,names,TRANGE=tr_jj
ps_quick_file,PREFIX=prefnm[0],SUFFIX=suffnm[0],TIME_LINES={UN:[tr_up,tr_dn],CO:[250L,250L,50L,50L]}
...
;;  Ti
get_data,Ti_name[0],DATA=ti_stru
;;  Vsw
...
;; => Load all relevant data
;;----------------------------------------------------------------------------------------
themis_load_all_inst,DATE=date[0],PROBE=probe[0],/LOAD_IESA_DF,/LOAD_EESA_DF,$
                     EESA_DF_OUT=eesa_out,IESA_DF_OUT=iesa_out,ESA_BF_TYPE='burst',$
                     TRANGE=time_double(tr_00)

peeb_df_arr_a  =  *(eesa_out.BURST[0])
peib_df_arr_a  =  *(iesa_out.BURST[0])
;;----------------------------------------------------------------------------------------
;; => Save DFs for later
;;----------------------------------------------------------------------------------------
sc      = probe[0]
...
SAVE,peeb_df_arr_a,FILENAME=enames[0]
SAVE,peib_df_arr_a,FILENAME=inames[0]
