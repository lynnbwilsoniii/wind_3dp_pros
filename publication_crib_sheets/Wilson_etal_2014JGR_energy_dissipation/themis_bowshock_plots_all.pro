...
tr_ww1         = time_double(tdate[0]+'/'+['15:53:09.165','15:53:12.500'])
...
;;----------------------------------------------------------------------------------------
coord          = 'gse'
IF (date EQ '072109') THEN nif_suffx      = '-RHS01'
IF (date EQ '072309') THEN nif_suffx      = '-RHS02'
IF (date EQ '092609') THEN nif_suffx      = '-RHS01'
coord_nif      = 'nif_S1986a'+nif_suffx[0]
mode_fgm       = ['fgs','fgl','fgh']
magname        = tnames(scpref[0]+mode_fgm+'_'+coord[0])
tpnm_fgm_bmag  = tnames(scpref[0]+mode_fgm+'_mag')
...
IF (date EQ '072109') THEN vel_suffx = 'Velocity_peib_no_GIs_UV_2'
IF (date EQ '072309') THEN vel_suffx = 'Velocity_peib_no_GIs_UV_2'
IF (date EQ '092609') THEN vel_suffx = '*Velocity_peib_no_GIs_UV_2'
den_suffx      = 'N_peib_no_GIs_UV'
IF (date EQ '072109') THEN itemp_suffx    = 'T_avg_peib_no_GIs_UV'
IF (date EQ '072309') THEN itemp_suffx    = 'T_avg_peib_no_GIs_UV'
IF (date EQ '092609') THEN itemp_suffx    = 'T_avg_peib_no_GIs_UV'
...
;;  For 2009-07-21
;;-----------------------------------------------------
IF (date EQ '072109') THEN Nic_yra  = [0d0,50d0]
IF (date EQ '072109') THEN Tic_yra  = [0d0,20d1]
IF (date EQ '072109') THEN Te__yra  = [0d0,40d0]
IF (date EQ '072109') THEN Vic_yra  = [-350d0,250d0]

IF (N_ELEMENTS(Nic_yra) NE 0 AND date EQ '072109') THEN options,N_i_core[0],'YRANGE',Nic_yra,/DEF
IF (N_ELEMENTS(Tic_yra) NE 0 AND date EQ '072109') THEN options,T_i_core[0],'YRANGE',Tic_yra,/DEF
IF (N_ELEMENTS(Te__yra) NE 0 AND date EQ '072109') THEN options,T_e_bulk[0],'YRANGE',Te__yra,/DEF
IF (N_ELEMENTS(Vic_yra) NE 0 AND date EQ '072109') THEN options,V_i_core[0],'YRANGE',Vic_yra,/DEF

IF (date EQ '072109') THEN tz_tra = t_ramp0[0] + [-1d0,5d-1]*4d2
...
IF (date EQ '072309') THEN tz_tra = REFORM(tzoom1[7,*]) + [-20d0,-10d0]
fnm_tra        = file_name_times(tz_tra,PREC=4)
...
;;  For 2009-09-26
;;-----------------------------------------------------
IF (date EQ '092609') THEN Nic_yra  = [0d0,80d0]
IF (date EQ '092609') THEN Tic_yra  = [0d0,20d1]
IF (date EQ '092609') THEN Te__yra  = [0d0,40d0]
IF (date EQ '092609') THEN Vic_yra  = [-350d0,100d0]

IF (date EQ '092609') THEN options,N_i_core[0],'MAX_VALUE',80d0,/DEF

IF (N_ELEMENTS(Nic_yra) NE 0 AND date EQ '092609') THEN options,N_i_core[0],'YRANGE',Nic_yra,/DEF
IF (N_ELEMENTS(Tic_yra) NE 0 AND date EQ '092609') THEN options,T_i_core[0],'YRANGE',Tic_yra,/DEF
IF (N_ELEMENTS(Te__yra) NE 0 AND date EQ '092609') THEN options,T_e_bulk[0],'YRANGE',Te__yra,/DEF
IF (N_ELEMENTS(Vic_yra) NE 0 AND date EQ '092609') THEN options,V_i_core[0],'YRANGE',Vic_yra,/DEF

IF (date EQ '092609') THEN tz_tra = t_ramp0[0] + [-1d0,1d0]*2d2
fnm_tra        = file_name_times(tz_tra,PREC=4)
ftime0         = fnm_tra.F_TIME[0]+'-'+STRMID(fnm_tra.F_TIME[1],11L)  ;; e.g., 2009-07-13_0859x41.8650-0859x51.8650
f_pref         = prefu[0]+'fgl-gse-mag_Nic_Tic_Te__Vic_ALL-Zooms_'
fname0         = f_pref[0]+ftime0+f_suffx0[0]
tzoom          = tzoom0
tramp          = t_ramp0[0]
...
IF (date EQ '092609') THEN tz_tra = REFORM(tzoom0[6,*])
IF (date EQ '092609') THEN tzstr0 = tz_strs[6]
fnm_tra        = file_name_times(tz_tra,PREC=4)
ftime0         = fnm_tra.F_TIME[0]+'-'+STRMID(fnm_tra.F_TIME[1],11L)  ;; e.g., 2009-07-13_0859x41.8650-0859x51.8650
f_pref         = prefu[0]+'fgh-gse-mag_ALL-Zooms_'
...
tpnm_fgm_bgse  = tnames(scpref[0]+mode_fgm+'_'+coord[0])
tpnm_fgm_bnif  = tnames(scpref[0]+mode_fgm+'_'+coord_nif[0])
fgs_gse_nm     = [tpnm_fgm_bmag[0],tpnm_fgm_bgse[0]]
fgl_gse_nm     = [tpnm_fgm_bmag[1],tpnm_fgm_bgse[1]]
fgh_gse_nm     = [tpnm_fgm_bmag[2],tpnm_fgm_bgse[2]]

efw_midnm      = mode_efi[1]+'_cal_corrected_DownSampled_'+coord_fac[0]
scw_midnm      = mode_scm[1]+'_cal_HighPass_'+coord_fac[0]
...
Edotj_names    = tnames(scpref[0]+Edotj_midnm[0]+'_sm*')       ;;  smoothed E.j
jvec_names     = tnames(scpref[0]+jvec_midnm[0]+'_sm*')        ;;  smoothed j-vector
jmag_names     = tnames(scpref[0]+jmag_midnm[0]+'_sm*')        ;;  smoothed |j|
...
IF (date EQ '092609') THEN fgm_mag_yra  = [0d0,50d0]
IF (date EQ '092609') THEN fgm_gse_yra  = [-45d0,30d0]
IF (date EQ '092609') THEN scm_fac_yra  = [-1d0,1d0]*1.75d0
IF (date EQ '092609') THEN efi_fac_yra  = [-1d0,1d0]*45d1
IF (date EQ '092609') THEN jvec_sm_yra  = [-1d0,1d0]*15d0
IF (date EQ '092609') THEN Edotjsm_yra  = [-1d0,1d0]*1.25d0

IF (N_ELEMENTS(fgm_mag_yra) NE 0 AND date EQ '092609') THEN options,tpnm_fgm_bmag,'YRANGE'
IF (N_ELEMENTS(fgm_mag_yra) NE 0 AND date EQ '092609') THEN options,tpnm_fgm_bmag,'YRANGE',fgm_mag_yra,/DEF
IF (N_ELEMENTS(fgm_gse_yra) NE 0 AND date EQ '092609') THEN options,tpnm_fgm_bgse,'YRANGE'
IF (N_ELEMENTS(fgm_gse_yra) NE 0 AND date EQ '092609') THEN options,tpnm_fgm_bgse,'YRANGE',fgm_gse_yra,/DEF
IF (N_ELEMENTS(scm_fac_yra) NE 0 AND date EQ '092609') THEN options,scw_names[1],'YRANGE'
IF (N_ELEMENTS(scm_fac_yra) NE 0 AND date EQ '092609') THEN options,scw_names[1],'YRANGE',scm_fac_yra,/DEF
IF (N_ELEMENTS(efi_fac_yra) NE 0 AND date EQ '092609') THEN options,efw_names[1],'YRANGE'
IF (N_ELEMENTS(efi_fac_yra) NE 0 AND date EQ '092609') THEN options,efw_names[1],'YRANGE',efi_fac_yra,/DEF
IF (N_ELEMENTS(Edotjsm_yra) NE 0 AND date EQ '092609') THEN options,Edotj_names[1],'YRANGE'
IF (N_ELEMENTS(Edotjsm_yra) NE 0 AND date EQ '092609') THEN options,Edotj_names[1],'YRANGE',Edotjsm_yra,/DEF
IF (N_ELEMENTS(jvec_sm_yra) NE 0 AND date EQ '092609') THEN options,jvec_names[0],'YRANGE'
IF (N_ELEMENTS(jvec_sm_yra) NE 0 AND date EQ '092609') THEN options,jvec_names[0],'YRANGE',jvec_sm_yra,/DEF


IF (date EQ '092609') THEN tz_tra = tr_ww1
IF (date EQ '092609') THEN tramp  = t_ramp_ra0[0]  ;;  accidentally used start of ramp to calculate distances for tick marks...
IF (date EQ '092609') THEN tzoom  = [[tr_esw0],[tr_esw1],[tr_esw2],[tr_whi]] ELSE tzoom = tzoom0
IF (date EQ '092609') THEN tzoom  = TRANSPOSE(tzoom)
fnm_tra        = file_name_times(tz_tra,PREC=4)
ftime0         = fnm_tra.F_TIME[0]+'-'+STRMID(fnm_tra.F_TIME[1],11L)  ;; e.g., 2009-07-13_0859x41.8650-0859x51.8650
f_pref         = prefu[0]+'fgh-gse-mag_scw-efw-FAC_Spow-NIF_jvec_neg-E.j_Wave-Zooms_'
fname0         = f_pref[0]+ftime0+f_suffx0[0]


nna            = [fgh_gse_nm,scw_names[1],efw_names[1],Spow_names[1],jvec_names[0],Edotj_names[1]]
  tplot,nna,TRANGE=tz_tra
  time_bar,REFORM(tzoom[*,0]),COLOR=250
  time_bar,REFORM(tzoom[*,1]),COLOR=250
  time_bar,tramp[0],COLOR=150
...
fnm_tra        = file_name_times(tz_tra,PREC=4)
ftime0         = fnm_tra.F_TIME[0]+'-'+STRMID(fnm_tra.F_TIME[1],11L)  ;; e.g., 2009-07-13_0859x41.8650-0859x51.8650
f_pref         = prefu[0]+'fgh-gse-mag_Zoom-to-Waveforms_'
fname0         = f_pref[0]+ftime0+tzstr0[0]+f_suffx0[0]
tzoom          = tr_ww1
tramp          = t_ramp0[0]
