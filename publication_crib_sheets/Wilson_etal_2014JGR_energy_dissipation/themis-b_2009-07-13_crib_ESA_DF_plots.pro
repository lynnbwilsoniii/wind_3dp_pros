...
;;----------------------------------------------------------------------------------------
;; => Plot angle map in DSL coordinates in spacecraft frame
;;----------------------------------------------------------------------------------------
...
sc         = probe[0]
pref       = 'th'+sc[0]+'_'
mag_dsl    = pref[0]+'fgs_'+coord[0]
fsuffx     = '_angle_map_sc-frame_'+STRUPCASE(coord[0])+'_'
j          = 195L

dat        = dat_i[j]
esaname    = 'IESA'
vsw_dsl    = pref[0]+'peib_velocity_'+coord[0]

dat        = dat_e[j]
esaname    = 'EESA'
vsw_dsl    = pref[0]+'peeb_velocity_'+coord[0]

;;  Make DSL vectors are defined in data structure
add_magf2,dat,mag_dsl[0],/LEAVE_ALONE
add_vsw2,dat,vsw_dsl[0],/LEAVE_ALONE

fnm        = file_name_times(dat[0].TIME,PREC=3)
ftimes     = fnm.F_TIME[0]
vsw        = dat.VSW
magf       = dat.MAGF
;; calculate [theta,phi] for each vector
cart_to_sphere,vsw[0],vsw[1],vsw[2],vmag,the_v,phi_v,PH_0_360=1
cart_to_sphere,magf[0],magf[1],magf[2],bmag,the_b,phi_b,PH_0_360=1
fout0      = '  For  '+esaname[0]+'  '+time_string(dat.TIME,PREC=3)
PRINT,';;', FLOAT(vmag[0]), FLOAT(the_v[0]), FLOAT(phi_v[0]), fout0[0]
PRINT,';;', FLOAT(bmag[0]), FLOAT(the_b[0]), FLOAT(phi_b[0]), fout0[0]
;;      248.838     -37.1892      188.486  For  IESA  2009-07-13/09:00:12.041
;;      3.94442      15.6232      221.895  For  IESA  2009-07-13/09:00:12.041
;;
;;      279.760     -27.8211      177.448  For  EESA  2009-07-13/09:00:12.041
;;      3.94442      15.6232      221.895  For  EESA  2009-07-13/09:00:12.041

;; create array at 360 deg intervals
phi_v2     = phi_v[0] + [0d0,1d0,2d0,3d0]*36d1
the_v2     = REPLICATE(the_v[0],4L)
phi_b2     = phi_b[0] + [0d0,1d0,2d0,3d0]*36d1
the_b2     = REPLICATE(the_b[0],4L)

fpref      = pref[0]+esaname[0]+fsuffx[0]
fname      = fpref[0]+ftimes[0]
  plot_map,dat[0],/DEFTICKS,COORD=STRUPCASE(coord[0])
  OPLOT,phi_v2,the_v2,PSYM=2,COLOR= 50L,SYMSIZE=3.0,THICK=2.0  ;; star at Vsw direction
  OPLOT,phi_b2,the_b2,PSYM=1,COLOR=250L,SYMSIZE=3.0,THICK=2.0  ;; plus at Bo  direction
popen,fname[0],/LAND
  plot_map,dat[0],/DEFTICKS,COORD=STRUPCASE(coord[0])
  OPLOT,phi_v2,the_v2,PSYM=2,COLOR= 50L,SYMSIZE=3.0,THICK=2.0  ;; star at Vsw direction
  OPLOT,phi_b2,the_b2,PSYM=1,COLOR=250L,SYMSIZE=3.0,THICK=2.0  ;; plus at Bo  direction
...
;;----------------------------------------------------------------------------------------
;; => Plot angle map in GSE coordinates in spacecraft frame
;;----------------------------------------------------------------------------------------
WINDOW,1,RETAIN=2,XSIZE=1200,YSIZE=800
...
sc         = probe[0]
pref       = 'th'+sc[0]+'_'
mag_gse    = pref[0]+'fgs_'+coord[0]
fsuffx     = '_angle_map_sc-frame_'+STRUPCASE(coord[0])+'_Corrected-Vbulk_'
j          = 195L
...
;;  Make DSL vectors are defined in data structure
add_magf2,dat,mag_gse[0],/LEAVE_ALONE
add_vsw2,dat,vsw_gse[0],/LEAVE_ALONE
...
PRINT,';;', FLOAT(vmag[0]), FLOAT(the_v[0]), FLOAT(phi_v[0]), fout0[0]
PRINT,';;', FLOAT(bmag[0]), FLOAT(the_b[0]), FLOAT(phi_b[0]), fout0[0]
;;      335.550      4.49444      169.877  For  IESA  2009-07-13/09:00:12.041
;;      3.94442     -18.7815      135.538  For  IESA  2009-07-13/09:00:12.041
;;
;;      335.550      4.49444      169.877  For  EESA  2009-07-13/09:00:12.041
;;      3.94442     -18.7815      135.538  For  EESA  2009-07-13/09:00:12.041
...
ni_ratio  = ni_halo/dens_lv2.Y[good]
...
esasufx   = [['N','T_avg','Velocity']+'_peib_no_GIs_UV']
nna       = pref[0]+[magsufx,esasufx]
fpref     = 'THB_fgl-mag-gse_Ni_Ti_Vi-Mom_corrected_level-2_'
