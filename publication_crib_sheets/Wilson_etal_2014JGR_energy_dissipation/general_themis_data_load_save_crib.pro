...
;;  Probe E
peeb_df_arr_e0 = *(poynter_peeb.BURST)[0]
peeb_df_arr_e1 = *(poynter_peeb.BURST)[1]
peeb_df_arr_e  = [peeb_df_arr_e0,peeb_df_arr_e1]

peib_df_arr_e0 = *(poynter_peib.BURST)[0]
peib_df_arr_e1 = *(poynter_peib.BURST)[1]
peib_df_arr_e2 = *(poynter_peib.BURST)[2]
peib_df_arr_e  = [peib_df_arr_e0,peib_df_arr_e1,peib_df_arr_e2]
;;  Sort by time
sp             = SORT(peeb_df_arr_e.TIME)
peeb_df_arr_e  = peeb_df_arr_e[sp]
sp             = SORT(peib_df_arr_e.TIME)
...
DELVAR,peeb_df_arr_c0,peeb_df_arr_c1,peib_df_arr_c0,peib_df_arr_c1,peib_df_arr_c2
DELVAR,peeb_df_arr_c,peib_df_arr_c
DELVAR,peeb_df_arr_e0,peeb_df_arr_e1,peib_df_arr_e0,peib_df_arr_e1,peib_df_arr_e2
DELVAR,peeb_df_arr_e,peib_df_arr_e
;;----------------------------------------------------------------------------------------
;; => Load E&B-Fields
;;----------------------------------------------------------------------------------------
mode           = 'efp efw'
...
efi_midnm      = '_cal_'
scm_midnm      = '_cal_'
diff_nm        = '('+modes_efi[1]+'-'+modes_efi[0]+')'
;;  EFI TPLOT handles
...
sratefw        = DOUBLE(sample_rate(efw_t,GAP_THRESH=6d0,/AVE))
se_wels        = t_interval_find(efw_t,GAP_THRESH=4d0/sratefw[0])
wint           = N_ELEMENTS(se_wels[*,0])
;;  Make sure each interval has enough elements
n_wels         = (se_wels[*,1] - se_wels[*,0]) LE 1000L
badn           = WHERE(n_wels,bdn)
test0          = (bdn GT 0) AND (badn[0] NE 0)
test1          = (bdn GT 0) AND (badn[0] EQ 0) AND (MAX(badn) NE wint)
IF (test0) THEN merge = [[badn - 1L],[badn]] ELSE merge = 0
IF (test1) THEN merge = [[badn],[badn + 1L]] ELSE merge = 0
IF (test0 OR test1) THEN nmer = N_ELEMENTS(merge[*,0]) ELSE nmer = 0
sz             = SIZE(se_wels,/DIMENSIONS)
se_weln        = se_wels
FOR j=0L, nmer - 1L DO BEGIN   $
  bels = REFORM(merge[j,*])  & $
  lels = se_wels[bels[0],0]  & $
  hels = se_wels[bels[1],1]  & $
  se_weln[bels,0] = lels     & $
  se_weln[bels,1] = hels
;;  keep only unique elements
unql           = UNIQ(se_weln[*,0],SORT(se_weln[*,0]))
sp             = SORT(unql)
unql           = unql[sp]
;;  redefine interval indices
se_wels        = se_weln[unql,*]
wint           = N_ELEMENTS(se_wels[*,0])

efw_v_nodc     = REPLICATE(d,SIZE(efw_v,/DIMENSIONS))
FOR j=0L, wint - 1L DO BEGIN                                                           $
  iels     = (se_wels[j,1] - se_wels[j,0]) - 999L                                    & $  ;; stay away from end pts
  st       = se_wels[j,0] + 500L                                                     & $
  en       = se_wels[j,1] - 500L                                                     & $
  inds     = st[0] + LINDGEN(en[0] - st[0] + 1L)                                     & $
  t_efwvx  = SMOOTH(efw_v[inds,0],10L,/NAN,/EDGE_TRUNCATE)                           & $
  t_efwvy  = SMOOTH(efw_v[inds,1],10L,/NAN,/EDGE_TRUNCATE)                           & $
  t_efwvz  = SMOOTH(efw_v[inds,2],10L,/NAN,/EDGE_TRUNCATE)                           & $
  t_efwv   = [[t_efwvx],[t_efwvy],[t_efwvz]]                                         & $
  avgs     = [MEAN(t_efwv[*,0],/NAN),MEAN(t_efwv[*,1],/NAN),MEAN(t_efwv[*,2],/NAN)]  & $
  FOR k=0L, 2L DO efw_v_nodc[inds,k] = efw_v[inds,k] - avgs[k]

;;  Send efw without DC-offsets to TPLOT
efi_nodcnm     = '_cal_rmDCoffsets_'
rmDC_nm        = 'efw(rm DC-offsets)'
nodc_tpnm      = pref[0]+modes_efi[1]+efi_nodcnm[0]+coord_dsl[0]
coord_up       = STRUPCASE(coord_dsl[0])
yttl           = 'E ['+rmDC_nm[0]+', '+coord_up[0]+', mV/m]'
str_element,dlim_efw,'YTITLE',yttl[0],/ADD_REPLACE
struct         = {X:efw_t,Y:efw_v_nodc}
store_data,nodc_tpnm[0],DATA=struct,DLIM=dlim_efw,LIM=lim_efw
...
;;  Define dummy arrays
efw_v_lowp     = REPLICATE(d,SIZE(efw_v,/DIMENSIONS))
efw_v_higp     = REPLICATE(d,SIZE(efw_v,/DIMENSIONS))
FOR j=0L, wint - 1L DO BEGIN                                                           $
  t_efwv   = efw_v_nodc[se_wels[j,0]:se_wels[j,1],*]                                 & $
  vec_lp   = vector_bandpass(t_efwv,srate[0],lowf0[0],lowf1[0],/MIDF)                & $
  vec_hp   = vector_bandpass(t_efwv,srate[0],midf[0],higf[0],/MIDF)                  & $
  efw_v_lowp[se_wels[j,0]:se_wels[j,1],*] = vec_lp                                   & $
  efw_v_higp[se_wels[j,0]:se_wels[j,1],*] = vec_hp

;;  Define TPLOT handles
efi_midnm      = '_cal_rmDCoffsets_'
low__suffx     = '_LowPassFilt_'+cutf_str[0]+'-'+lowf_str[0]+'Hz'
;low__suffx     = '_LowPassFilt_'+lowf_str[0]+'Hz'
high_suffx     = '_HighPassFilt_'+midf_str[0]+'Hz'
low__tpnm      = pref[0]+modes_efi[1]+efi_midnm[0]+coord_dsl[0]+low__suffx[0]
high_tpnm      = pref[0]+modes_efi[1]+efi_midnm[0]+coord_dsl[0]+high_suffx[0]
...
str_element,dlim_efw,'YTITLE',yttl[0],/ADD_REPLACE
str_element,dlim_efw,'YSUBTITLE',yttl_sub_lp[0],/ADD_REPLACE
struct         = {X:efw_t,Y:efw_v_lowp}
store_data,low__tpnm[0],DATA=struct,DLIM=dlim_efw,LIM=lim_efw

str_element,dlim_efw,'YSUBTITLE',yttl_sub_hp[0],/ADD_REPLACE
struct         = {X:efw_t,Y:efw_v_higp}
store_data,high_tpnm[0],DATA=struct,DLIM=dlim_efw,LIM=lim_efw

;;  Add note to explain 10 Hz cutoff at low end
cutf_str       = STRTRIM(STRING(lowf0[0],FORMAT='(I)'),2L)
...
scw_t          = temp_scw.X
scw_v          = temp_scw.Y
;;  Define sample rate and find interval start/end elements
sratscw        = DOUBLE(sample_rate(scw_t,GAP_THRESH=6d0,/AVE))
se_vels        = t_interval_find(scw_t,GAP_THRESH=4d0/sratscw[0])
vint           = N_ELEMENTS(se_vels[*,0])
;;  Make sure each interval has enough elements
n_vels         = (se_vels[*,1] - se_vels[*,0]) LE 1000L
badn           = WHERE(n_vels,bdn)
test0          = (bdn GT 0) AND (badn[0] NE 0)
test1          = (bdn GT 0) AND (badn[0] EQ 0) AND (MAX(badn) NE vint)
IF (test0) THEN merge = [[badn - 1L],[badn]] ELSE merge = 0
IF (test1) THEN merge = [[badn],[badn + 1L]] ELSE merge = 0
IF (test0 OR test1) THEN nmer = N_ELEMENTS(merge[*,0]) ELSE nmer = 0
sz             = SIZE(se_vels,/DIMENSIONS)
se_veln        = se_vels
FOR j=0L, nmer - 1L DO BEGIN   $
  bels = REFORM(merge[j,*])  & $
  lels = se_vels[bels[0],0]  & $
  hels = se_vels[bels[1],1]  & $
  se_veln[bels,0] = lels     & $
  se_veln[bels,1] = hels
;;  keep only unique elements
unql           = UNIQ(se_veln[*,0],SORT(se_veln[*,0]))
sp             = SORT(unql)
unql           = unql[sp]
;;  redefine interval indices
se_vels        = se_veln[unql,*]
vint           = N_ELEMENTS(se_vels[*,0])
;;  Define cut-off frequencies for bandpass filters
lowf0          = 1d1
lowf1          = 1d2
midf           = 5d2
higf           = 1d5   ;;  Set well above Nyquist frequency
;;  Define the sample rate [sps]
srate          = sratscw[0]
;;  Define strings associated with sample rates and filters
cutf_str       = STRTRIM(STRING(lowf0[0],FORMAT='(I)'),2L)
sr_str         = STRTRIM(STRING(srate[0],FORMAT='(I)'),2L)
lowf_str       = STRTRIM(STRING(lowf1[0],FORMAT='(I)'),2L)
midf_str       = STRTRIM(STRING(midf[0],FORMAT='(I)'),2L)
;;  Define dummy arrays and apply filters
scw_v_lowp     = REPLICATE(d,SIZE(scw_v,/DIMENSIONS))
scw_v_higp     = REPLICATE(d,SIZE(scw_v,/DIMENSIONS))
scw_v_nodc     = scw_v
FOR j=0L, vint - 1L DO BEGIN                                                           $
  t_scwv   = scw_v_nodc[se_vels[j,0]:se_vels[j,1],*]                                 & $
  vec_lp   = vector_bandpass(t_scwv,srate[0],lowf0[0],lowf1[0],/MIDF)                & $
  vec_hp   = vector_bandpass(t_scwv,srate[0],midf[0],higf[0],/MIDF)                  & $
  scw_v_lowp[se_vels[j,0]:se_vels[j,1],*] = vec_lp                                   & $
  scw_v_higp[se_vels[j,0]:se_vels[j,1],*] = vec_hp

;;  Define TPLOT handles
low__suffx     = '_LowPassFilt_'+cutf_str[0]+'-'+lowf_str[0]+'Hz'
;low__suffx     = '_LowPassFilt_'+lowf_str[0]+'Hz'
high_suffx     = '_HighPassFilt_'+midf_str[0]+'Hz'
low__tpnm      = pref[0]+modes_scm[1]+scm_midnm[0]+coord_dsl[0]+low__suffx[0]
high_tpnm      = pref[0]+modes_scm[1]+scm_midnm[0]+coord_dsl[0]+high_suffx[0]
coord_up       = STRUPCASE(coord_dsl[0])

yttl           = 'B ['+modes_scm[1]+', '+coord_up[0]+', nT]'
yttl_sub_lp    = '['+sr_str[0]+' sps, LP: '+lowf_str[0]+'Hz]'
yttl_sub_hp    = '['+sr_str[0]+' sps, HP: '+midf_str[0]+'Hz]'
;;  Send to TPLOT
str_element,dlim_scw,'YTITLE',yttl[0],/ADD_REPLACE
str_element,dlim_scw,'YSUBTITLE',yttl_sub_lp[0],/ADD_REPLACE
struct         = {X:scw_t,Y:scw_v_lowp}
store_data,low__tpnm[0],DATA=struct,DLIM=dlim_scw,LIM=lim_scw

str_element,dlim_scw,'YSUBTITLE',yttl_sub_hp[0],/ADD_REPLACE
struct         = {X:scw_t,Y:scw_v_higp}
store_data,high_tpnm[0],DATA=struct,DLIM=dlim_scw,LIM=lim_scw
...
srate          = sratscw[0]
;;  Define strings associated with sample rates and filters
cutf_str       = STRTRIM(STRING(lowf0[0],FORMAT='(f3.1)'),2L)
sr_str         = STRTRIM(STRING(srate[0],FORMAT='(I)'),2L)
lowf_str       = STRTRIM(STRING(lowf1[0],FORMAT='(I)'),2L)
midf_str       = STRTRIM(STRING(midf[0],FORMAT='(I)'),2L)
;;  Define dummy arrays and apply filters
scw_v_low2     = REPLICATE(d,SIZE(scw_v,/DIMENSIONS))
scw_v_midp     = REPLICATE(d,SIZE(scw_v,/DIMENSIONS))
scw_v_nodc     = scw_v
FOR j=0L, vint - 1L DO BEGIN                                                           $
  t_scwv   = scw_v_nodc[se_vels[j,0]:se_vels[j,1],*]                                 & $
  vec_lp   = vector_bandpass(t_scwv,srate[0],lowf0[0],lowf1[0],/MIDF)                & $
  vec_hp   = vector_bandpass(t_scwv,srate[0],lowf1[0],midf[0],/MIDF)                 & $
  scw_v_low2[se_vels[j,0]:se_vels[j,1],*] = vec_lp                                   & $
  scw_v_midp[se_vels[j,0]:se_vels[j,1],*] = vec_hp

;;  Define TPLOT handles
low__suffx     = '_LowPassFilt_'+cutf_str[0]+'-'+lowf_str[0]+'Hz'
high_suffx     = '_BandPassFilt_'+lowf_str[0]+'-'+midf_str[0]+'Hz'
low__tpnm      = pref[0]+modes_scm[1]+scm_midnm[0]+coord_dsl[0]+low__suffx[0]
high_tpnm      = pref[0]+modes_scm[1]+scm_midnm[0]+coord_dsl[0]+high_suffx[0]
coord_up       = STRUPCASE(coord_dsl[0])

yttl           = 'B ['+modes_scm[1]+', '+coord_up[0]+', nT]'
yttl_sub_lp    = '['+sr_str[0]+' sps, LP: '+cutf_str[0]+'-'+lowf_str[0]+' Hz]'
yttl_sub_hp    = '['+sr_str[0]+' sps, BP: '+lowf_str[0]+'-'+midf_str[0]+' Hz]'
;;  Send to TPLOT
str_element,dlim_scw,'YTITLE',yttl[0],/ADD_REPLACE
str_element,dlim_scw,'YSUBTITLE',yttl_sub_lp[0],/ADD_REPLACE
struct         = {X:scw_t,Y:scw_v_low2}
store_data,low__tpnm[0],DATA=struct,DLIM=dlim_scw,LIM=lim_scw

str_element,dlim_scw,'YSUBTITLE',yttl_sub_hp[0],/ADD_REPLACE
struct         = {X:scw_t,Y:scw_v_midp}
store_data,high_tpnm[0],DATA=struct,DLIM=dlim_scw,LIM=lim_scw

;;  Add note to explain 0.1 Hz cutoff at low end
cutf_str       = STRTRIM(STRING(lowf0[0],FORMAT='(I)'),2L)
opt_snm        = 'DATA_ATT.NOTE_FILT'
opt_str_lp     = '[Filtered:  '+cutf_str[0]+'-'+lowf_str[0]+' Hz]'
opt_str_hp     = '[Filtered:  '+lowf_str[0]+'-'+midf_str[0]+' Hz]'
nna            = [low__tpnm[0],high_tpnm[0]]
options,nna[0],opt_snm[0],opt_str_lp[0],/DEF
options,nna[1],opt_snm[0],opt_str_hp[0],/DEF
