...
;;  Insert NaNs for the ESA data (makes it easier to distinguish data gaps)
tpname         = [vbulk_tpn[0],densi_tpn[0],eitem_tpn]
t_insert_nan_at_interval_se,tpname,GAP_THRESH=6d0
...
efp_fac_tpns   = tnames(scpref[0]+'efp_l1_cal_*_corrected_rmspikes_zdsl_'+coord_fac[0])
scp_fac_tpns   = tnames(scpref[0]+'scp_l1_cal_*_zdsl_'+coord_fac[0])
efw_fac_tpns   = tnames(scpref[0]+'efw_l1_cal_*_corrected_rmspikes_zdsl_'+coord_fac[0])
...
;file_out       = tpn__file[0]+'_with_eVDF_fit_results'
file_out       = STRMID(tpn__file[0],0L,STRLEN(tpn__file[0])-6L)+'_with_eVDF_fit_results'
tplot_save,FILENAME=file_out[0]   ;;  tpn__file defined in load_thm_fgm_efi_scm_2_tplot_batch.pro
