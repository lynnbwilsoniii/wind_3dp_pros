...
;               DUMB       :  Copy of DAT but THETA and PHI structure tags are still in
;                               DSL coordinates
;               BHTR_GSE   :  Scalar [string] defining the TPLOT handle containing the
...
;               SPPERI     :  Scalar [string] defining the TPLOT handle containing the
;                               spin period [seconds] of the associated spacecraft
;
;  EXAMPLES:    
;               
;
...
;
;   CHANGED:  1)  rotate_esa_thetaphi_to_gse.pro can now handle single data structures
;                   so the copies are not necessary prior to calling
;                                                                  [08/11/2012   v1.1.0]
;             2)  Now routine assumes structures have already been modified by
;                   modify_themis_esa_struc.pro and rotate_esa_thetaphi_to_gse.pro but
;                   user must now send in a dummy copy still in DSL coordinates and
;                   now outputs crosshairs corresponding to where cuts were made and
;                   now calls get_font_symbol.pro and get_color_by_name.pro and
;                   no longer calls cgColor.pro
;                                                                  [10/31/2012   v1.2.0]
;
;   NOTES:      
;               1)  HTR = High Time Resolution
;               2)  The routine assumes that the input structure angles are in GSE
;                     coordinates
;               3)  On input, DAT should have the following tags defined:
;                     VSW  = bulk flow velocity [km/s, GSE]
;                     MAGF = average magnetic field vector [nT, GSE]
;               4)  See also:  contour_3d_htr_1plane.pro or contour_3d_1plane.pro
;
;   CREATED:  08/08/2012
;   CREATED BY:  Lynn B. Wilson III
;    LAST MODIFIED:  10/31/2012   v1.2.0
;    MODIFIED BY: Lynn B. Wilson III
;
;*****************************************************************************************
;-

PRO contour_esa_htr_1plane,dat,dumb,bhtr_gse,vector2,spperi,VLIM=vlim,NGRID=ngrid,   $
...
FORWARD_FUNCTION routine_version
...
ENDIF
;;----------------------------------------------------------------------------------------
;; => Check DAT and DUMB structure format
;;----------------------------------------------------------------------------------------
...
IF (test) THEN BEGIN
  ...
  RETURN
ENDIF
test           = test_themis_esa_struc_format(dumb,/NOM) NE 1
IF (test) THEN BEGIN
  MESSAGE,notstr_msg[0],/INFORMATIONAL,/CONTINUE
  ;; => Create empty plot
  !P.MULTI = 0
...
data           = dat[0]
tad            = dumb[0]  ;; dummy copy [with DSL angles] for later use
...
ENDIF
;;----------------------------------------------------------------------------------------
;; => Make sure VSW and MAGF structure tags are defined
;;----------------------------------------------------------------------------------------
add_magf2,data,bhtr_gse[0],/LEAVE_ALONE
IF KEYWORD_SET(vel_name) THEN BEGIN
  get_data,vel_name[0],DATA=test_vel
  test_v         = TOTAL(FINITE(data[0].VSW)) NE 3
  IF (SIZE(test_vel,/TYPE) EQ 8L AND test_v) THEN add_vsw2,data,vel_name[0]
ENDIF
;;----------------------------------------------------------------------------------------
;; => Rotate DAT structure (theta,phi)-angles DSL --> GSE
;;----------------------------------------------------------------------------------------
;;  the following has trouble with only one structure
;rotate_esa_thetaphi_to_gse,data,MAGF_NAME=mag_name,VEL_NAME=vel_name
;IF KEYWORD_SET(one_c) THEN BEGIN
;  ;; => Rotate DAT structure (theta,phi)-angles DSL --> GSE
;  rotate_esa_thetaphi_to_gse,onec,MAGF_NAME=mag_name,VEL_NAME=vel_name
;ENDIF
;;----------------------------------------------------------------------------------------
...
;;  Check format of VEC1 and VEC2
test_v1  = TOTAL(FINITE(vec1)) NE 3
test_v2  = TOTAL(FINITE(vec2)) NE 3
;;----------------------------------------------------------------------------------------
...
;;----------------------------------------------------------------------------------------
IF (test_v1) THEN vec1 = data[0].MAGF
IF (test_v2) THEN vec2 = data[0].VSW
...
IF (SIZE(onec,/TYPE) EQ 8) THEN BEGIN
  ...
  test_v1  = TOTAL(FINITE(vec1)) NE 3
  test_v2  = TOTAL(FINITE(vec2)) NE 3
  IF (test_v1) THEN vec1 = onec[0].MAGF
  IF (test_v2) THEN vec2 = onec[0].VSW
