...
;                                                                  [08/08/2012   v1.1.1]
;             3)  If N = 1, the routine has issues, so now routine makes copies to
;                   prevent this issue
;                                                                  [08/11/2012   v1.2.0]
;
;   NOTES:      
...
IF (n_d EQ 1L) THEN BEGIN
  ;; => Only 1 distribution
  ;;    => Make copies
  data  = REPLICATE(dat[0],5L)
  only1 = 1                                         ;; logic for later
ENDIF ELSE BEGIN
  ;; => More than 1 distribution
  data  = dat                                       ;; => dummy for later
  only1 = 0
ENDELSE
n_d            = N_ELEMENTS(data)                   ;; # of data structures
...
mass           = data[0].MASS                       ;; mass of particle [eV km^(-2) s^(2)]
