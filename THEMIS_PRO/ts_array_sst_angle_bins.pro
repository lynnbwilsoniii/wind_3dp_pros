...
;*****************************************************************************************
;
;  FUNCTION :   ts_array_sst_angle_bins.pro
;  PURPOSE  :   This routine determines the Unix time stamps associated with each angle
;                 bin in an array of THEMIS SST velocity distribution functions (VDFs)
...
;                               a() known THEMIS SST IDL data structure(s)
;                               [e.g., see get_th?_p*@b.pro, ? = a-f, * = s @ = e,i]
;               SPPERI     :  Scalar (or [N]-element array) [numeric] of spacecraft body
;                               spin rate(s) [deg/s] at the time(s) of DAT
;
;  EXAMPLES:    
;               [calling sequence]
;               t_phi_sst = ts_array_esa_angle_bins(dat, spperi)
;
;  KEYWORDS:    
;               NA
;
...
;
;   NOTES:      
;               1)  See also:  timestamp_esa_angle_bins.pro and ts_array_esa_angle_bins.pro
...
;               6)  Ni, B., Y. Shprits, M. Hartinger, V. Angelopoulos, X. Gu, and
...
;               7)  Turner, D.L., V. Angelopoulos, Y. Shprits, A. Kellerman, P. Cruce,
...
FUNCTION ts_array_sst_angle_bins,dat,spperi
...
badvdf_msg     = 'Input must be an IDL structure from one of the THEMIS SST instruments...'
;;----------------------------------------------------------------------------------------
;;  Check input
;;----------------------------------------------------------------------------------------
...
;;  Define SST-dependent variables
n_sst          = N_ELEMENTS(dat0)                   ;;  Number of SST data structures
...
t_zero_sst     = REFORM(phi[0,0,*])/sp_rate         ;;  [N]-Element array of initial time offset [s] from start of VDF
;;  Define dummy arrays to fill later
dt_sst         = DBLARR(n_e[0],n_a[0],n_sst[0])     ;;  Total accumulation times [s] for each SST bin
t_off_av_sst   = DBLARR(n_e[0],n_a[0],n_sst[0])     ;;  Time offsets [s] of each energy/angle bin
dt_off_st_sst  = DBLARR(n_e[0],n_a[0],n_sst[0])     ;;  Time offsets [s] adjusted by accumulation times
t_phi_sst      = DBLARR(n_e[0],n_a[0],n_sst[0])     ;;  Time stamps [Unix] of every energy/angle bin for each VDF
;;----------------------------------------------------------------------------------------
;;  Define offsets by azimuthal angles [deg, DSL]
;;----------------------------------------------------------------------------------------
dt_sst         = DOUBLE(dat0.INTEG_T)               ;;  [E,A,N]-Element array of total accumulation times [s]
