...
;                             [Default = FALSE]
;
;   CHANGED:  1)  Added keyword:  FORCE_RA
;                                                                   [09/11/2014   v1.1.0]
;
;   NOTES:      
...
;    LAST MODIFIED:  09/11/2014   v1.1.0
;    MODIFIED BY: Lynn B. Wilson III
;
;*****************************************************************************************
;-

FUNCTION log10_tickmarks,data,RANGE=range,MIN_VAL=min_val,MAX_VAL=max_val,FORCE_RA=force_ra
...
;;    --> LBW III  09/11/2014   v1.1.0
  IF (range[0] EQ range[1]) THEN BEGIN
    dra = allowed_ran
  ENDIF ELSE BEGIN
    dra = REFORM(range[SORT(range)])
    set_bounds[*] = 1b
  ENDELSE
ENDELSE
;;----------------------------------------------------------------------------------------
...
;;  Check FORCE_RA keyword
IF KEYWORD_SET(force_ra) THEN BEGIN
  good  = REPLICATE(1b,nlt[0])
  tests = {T0:(tick_vals LT dra[0]),T1:(tick_vals GT dra[1])}
  badl  = WHERE(REPLICATE(set_bounds[0],nlt[0]) AND tests.(0),bdl)
  badh  = WHERE(REPLICATE(set_bounds[1],nlt[0]) AND tests.(1),bdh)
  IF (bdl[0] GT 0) THEN good[badl] = 0b
  IF (bdh[0] GT 0) THEN good[badh] = 0b
  test  = (TOTAL(good) GT 0)
  IF (test[0]) THEN BEGIN
    ;;  Tick marks were found outside bounds
    ;;    --> Remove
    good       = WHERE(good)
    tick_vals  = tick_vals[good]
    tick_names = tick_names[good]
    pow        = pow[good]
    nlt        = N_ELEMENTS(tick_vals)
  ENDIF
ENDIF
