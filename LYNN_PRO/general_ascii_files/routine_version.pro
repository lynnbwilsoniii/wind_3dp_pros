...
;                                                                   [10/01/2015   v1.1.0]
;             3)  Now checks for starting ;+ and trailing ;- characters to ensure
;                   the main routine's Man. page is being read and not a subroutine
;                   within
;                                                                   [08/24/2017   v1.2.0]
;
;   NOTES:      
...
;   CREATED:  08/08/2012
;   CREATED BY:  Lynn B. Wilson III
;    LAST MODIFIED:  08/24/2017   v1.2.0
;    MODIFIED BY: Lynn B. Wilson III
;
;*****************************************************************************************
;-
...
IF (file[0] NE '') THEN BEGIN
  ...
  ;;  First look for starting ;+ and trailing ;- characters
  test     = STRPOS(fstring,';+') GE 0
  gposi    = WHERE(test,gpf)
  nf       = N_ELEMENTS(fstring)
  imax     = nf[0] - 1L
  IF (gpf[0] GT 0) THEN imin = gposi[0] ELSE imin = 0L
  f_list   = fstring[imin[0]:imax[0]]
  test     = STRPOS(f_list,';    LAST MODIFIED:  ') GE 0
  gposi    = WHERE(test,gpf)
  shifts   = STRLEN(';    LAST MODIFIED:  ')
  IF (gpf[0] GT 0) THEN vers = STRMID(f_list[gposi[0]],shifts[0]) ELSE vers = '(Not Found)'
