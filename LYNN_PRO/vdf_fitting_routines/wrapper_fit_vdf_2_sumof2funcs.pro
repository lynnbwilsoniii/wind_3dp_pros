...
;    LAST MODIFIED:  04/24/2018   v1.0.1
...
;   CREATED:  04/17/2018
;   CREATED BY:  Lynn B. Wilson III
;    LAST MODIFIED:  04/24/2018   v1.2.3
;    MODIFIED BY: Lynn B. Wilson III
;
;*****************************************************************************************
...
def_kapp_lim            = [3d0/2d0,90d0]  ;;  3/2 ≤ kappa ≤ 90
...
def_pinf[00].LIMITS[*]  = lims_ncore
def_pinf[06].LIMITED[*] = limd_nhalo
def_pinf[06].LIMITS[*]  = lims_nhalo
def_binf[00].LIMITED[*] = limd_nbeam
def_binf[00].LIMITS[*]  = lims_nbeam
...
;;;----------------------------------------------------------------------------------------
;;;  Let derivatives for densities be explicit
;;;----------------------------------------------------------------------------------------
def_pinf[00].MPSIDE[0]  = 3
def_pinf[06].MPSIDE[0]  = 3
def_binf[00].MPSIDE[0]  = 3
;;----------------------------------------------------------------------------------------
...
;   CREATED:  04/09/2018
;   CREATED BY:  Lynn B. Wilson III
;    LAST MODIFIED:  04/24/2018   v1.2.4
;    MODIFIED BY: Lynn B. Wilson III
;
;*****************************************************************************************
;-
...
bad                     = WHERE(FINITE(vdf_2dc) EQ 0 OR vdf_2dc LE 0,bd,COMPLEMENT=good,NCOMPLEMENT=gd)
...
IF (ch_emin[0] GT 0) THEN BEGIN
  ...
  vpar2d                  = vpara # REPLICATE(1d0,n_per[0])
  vper2d                  = REPLICATE(1d0,n_par[0]) # vperp
  ...
  eners                   = energy_to_vel(speed,ELECTRON=electrons,PROTON=ions,/INVERSE)
...
IF (status[0] GT 0) THEN BEGIN  ;;  TRUE = Success!  -->  Print out info
  ...
  fit_out                 = vdf_fit_out
  residual                = (vdf_2db - fit_out) > 0             ;;  Force values to > 1.0
  ...
  IF (b__emin[0] GT 0) THEN BEGIN
    ;;  First see if SPEED and ENERS variables were already created (i.e., EMIN_CH was set and used)
    test                    = (N_ELEMENTS(speed) NE n_par[0]*n_per[0])
    IF (test[0]) THEN BEGIN
      vpar2d                  = vpara # REPLICATE(1d0,n_per[0])
      vper2d                  = REPLICATE(1d0,n_par[0]) # vperp
      vel_2d                  = [[[vpar2d]],[[vper2d]]]             ;;  [N,M,2]-Element array
      speed                   = SQRT(TOTAL(vel_2d^2,3,/NAN))        ;;  [N,M]-Element array of speeds [km/s]
    ENDIF
    test                    = (N_ELEMENTS(eners) NE n_par[0]*n_per[0])
    IF (test[0]) THEN BEGIN
      ;;  Convert to energies [eV]
      eners                   = energy_to_vel(speed,ELECTRON=electrons,PROTON=ions,/INVERSE)
    ENDIF
  ...
  ENDIF ELSE BEGIN
    ...
    IF (SIZE(b_sigma,/TYPE) EQ 0)          THEN bsigp       = REPLICATE(d,6)                 ELSE bsigp    = b_sigma/(1d-2*offset[0])
...
ENDIF ELSE BEGIN
  ...
  IF (SIZE(vdf_fit_out,/TYPE) EQ 0)      THEN fit_out     = REPLICATE(d,n_par[0],n_per[0]) ELSE fit_out  = vdf_fit_out
  IF (SIZE(bifit,/TYPE) EQ 0)            THEN bifitout    = REPLICATE(d,12)                ELSE bifitout = bifit
  IF (SIZE(f_sigma,/TYPE) EQ 0)          THEN f_sigout    = REPLICATE(d,12)                ELSE f_sigout = f_sigma/(1d-2*offset[0])
