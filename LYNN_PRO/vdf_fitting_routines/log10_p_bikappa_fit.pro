...
;*****************************************************************************************
;
;  FUNCTION :   log10_p_bikappa_fit.pro
;  PURPOSE  :   Creates a Bi-Kappa Distribution Function (KDF) for protons from a user
...
;  EXAMPLES:    
;               [calling sequence]
;               l10bikap = log10_p_bikappa_fit(vpara, vperp, param, pder [,FF=ff])
;
;  KEYWORDS:    
...
FUNCTION log10_p_bikappa_fit,vpara,vperp,param,pder,FF=ff
...
factor2        = param[0]/(vtppar[0]*vtpper[0]^2d0)
  ...
  FOR k=0L, np[0] - 1L DO BEGIN
    IF (requested[k] NE 0) THEN BEGIN
      CASE k[0] OF
        ...
        1     :  BEGIN
          ;;  dZ/dB = Z [2 K1p W_par^2 - W_per^2 - K32]/[B * DEN]
          pder[*,*,k] = ALOG10(ABS(ff*(2d0*kp_1[0]*welpa2d^2d0 - welpe2d^2d0 - km32[0])/(vtppar[0]*denom)))
        END
        2     :  BEGIN
          ;;  dZ/dC = Z 2*[F W_per^2 - W_par^2 - K32]/[C * DEN]
          pder[*,*,k] = ALOG10(ABS(ff*2d0*(kk[0]*welpe2d^2d0 - welpa2d^2d0 - km32[0])/(vtpper[0]*denom)))
        END
        3     :  BEGIN
          ;;  dZ/dD = Z 2*[W_par K_1]/[B * DEN]
          pder[*,*,k] = ALOG10(ABS(ff*2d0*welpa2d*kp_1[0]/(vtppar[0]*denom)))
        END
        4     :  BEGIN
          ;;  dZ/dE = Z 2*[W_per K_1]/[C * DEN]
          pder[*,*,k] = ALOG10(ABS(ff*2d0*welpe2d*kp_1[0]/(vtpper[0]*denom)))
        END
        5     :  BEGIN
          ;;  dZ/dF = Z { [(W_par^2 + W_per^2) K1m - K32*3/2]/[K32 * DEN] -
          ;;             ln| 1 + (W_par^2 + W_per^2)/K32 | + DG(K_1) - DG(K1m) }
