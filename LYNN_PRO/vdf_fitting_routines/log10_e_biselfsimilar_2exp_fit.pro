...
;*****************************************************************************************
;
;  FUNCTION :   log10_e_biselfsimilar_2exp_fit.pro
...
;  EXAMPLES:    
;               [calling sequence]
;               l10biss2exp = log10_e_biselfsimilar_2exp_fit(vpara, vperp, param, pder [,FF=ff])
;
;  KEYWORDS:    
...
FUNCTION log10_e_biselfsimilar_2exp_fit,vpara,vperp,param,pder,FF=ff
  ...
  FOR k=0L, np[0] - 1L DO BEGIN
    IF (requested[k] NE 0) THEN BEGIN
      CASE k[0] OF
        0     :  BEGIN
          ;;  dZ/dA = Z/A
          pder[*,*,k] = ALOG10(ff/param[0])
        END
        1     :  BEGIN
          ;;  dZ/dB = Z [F W_par^(E) - 1]/B
          pder[*,*,k] = ALOG10(ff*(pp[0]*w_para - 1d0)/vtepar[0])
        END
        2     :  BEGIN
          ;;  dZ/dC = Z [F W_per^(F) - 2]/C
          pder[*,*,k] = ALOG10(ff*(qq[0]*w_perp - 2d0)/vteper[0])
        END
        3     :  BEGIN
          ;;  dZ/dD = Z [F W_par^(E-1)]/B
          even_t      = ((pp[0] - 1d0) MOD 2) EQ 0
          IF (even_t[0]) THEN term0 = welpa2d^(pp[0] - 1d0) ELSE term0 = ABS(welpa2d)^(pp[0] - 1d0)
          pder[*,*,k] = ALOG10(ff*(pp[0]*term0)/vtepar[0])
        END
        4     :  BEGIN
          ;;  dZ/dE = Z { [DG(1 + 1/E)/E^2] - W_par^(E) Ln|W_par| }
          even_t      = ((pp[0] - 1d0) MOD 2) EQ 0
