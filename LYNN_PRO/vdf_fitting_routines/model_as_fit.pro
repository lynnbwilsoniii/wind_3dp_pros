...
;                 of velocities have the same units.  This function can be asymmetric in
;                 the exponent and/or the thermal speeds.
;
;  CALLED BY:   
;               NA
;
...
;  EXAMPLES:    
;               [calling sequence]
;               biss2exp = model_as_fit(vpara, vperp, param, pder [,FF=ff])
;
;  KEYWORDS:    
...
FUNCTION model_as_fit,vpara,vperp,parm0,pder,FF=ff
...
;;  Load Common Block
;;----------------------------------------------------------------------------------------
par_facs       = fit_parm_fact_get_common(/GET_AS,STATUS=status,INITYES=inityes)
