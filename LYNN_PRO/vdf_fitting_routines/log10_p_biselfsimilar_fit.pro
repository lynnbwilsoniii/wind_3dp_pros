...
;*****************************************************************************************
;
;  FUNCTION :   log10_p_biselfsimilar_fit.pro
...
;  EXAMPLES:    
;               [calling sequence]
;               l10bissm = log10_p_biselfsimilar_fit(vpara, vperp, param, pder [,FF=ff])
;
;  KEYWORDS:    
...
FUNCTION log10_p_biselfsimilar_fit,vpara,vperp,param,pder,FF=ff
  ...
  FOR k=0L, np[0] - 1L DO BEGIN
    IF (requested[k] NE 0) THEN BEGIN
      CASE k[0] OF
        ...
        2     :  BEGIN
          ;;  dZ/dC = Z [F W_per^(F) - 2]/C
          pder[*,*,k] = ALOG10(ABS(ff*(pp[0]*w_perp - 2d0)/vtpper[0]))
        END
        3     :  BEGIN
          ;;  dZ/dD = Z [F W_par^(F-1)]/B
        ...
        4     :  BEGIN
          ...
          pder[*,*,k] = ALOG10(ABS(ff*(pp[0]*term0)/vtpper[0]))
        END
        5     :  BEGIN
          ;;  dZ/dF = Z { [3 DG(1 + 1/F)/F^2] - W_par^(F) Ln|W_par| - W_per^(F) Ln|W_per|}
          ndg         = 30000000L
