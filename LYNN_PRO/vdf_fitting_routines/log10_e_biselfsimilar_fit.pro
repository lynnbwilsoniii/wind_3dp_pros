...
;*****************************************************************************************
;
;  FUNCTION :   log10_e_biselfsimilar_fit.pro
;  PURPOSE  :   Creates an symmetric bi-self-similar (SS) distribution function for
;                 electrons from a user defined amplitude, temperature, and array of
;                 velocities.  The only note to be careful of is to make sure the thermal
;                 speed and array of velocities have the same units.  The output will
;                 be the base-10 log of the SS.
;
;  CALLED BY:   
;               NA
;
...
;  EXAMPLES:    
;               [calling sequence]
;               l10bissm = log10_e_biselfsimilar_fit(vpara, vperp, param, pder [,FF=ff])
;
;  KEYWORDS:    
...
;
;   ADAPTED FROM: biselfsimilar_fit.pro    BY: Lynn B. Wilson III
;   CREATED:  04/03/2019
...
FUNCTION log10_e_biselfsimilar_fit,vpara,vperp,param,pder,FF=ff
...
amp            = factor[0]*param[0]/(vtepar[0]*vteper[0]^2d0)
  ...
  FOR k=0L, np[0] - 1L DO BEGIN
    IF (requested[k] NE 0) THEN BEGIN
      CASE k[0] OF
        ...
        1     :  BEGIN
          ;;  dZ/dB = Z [F W_par^(F) - 1]/B
          pder[*,*,k] = ALOG10(ABS(ff*(pp[0]*w_para - 1d0)/vtepar[0]))
        END
        2     :  BEGIN
          ;;  dZ/dC = Z [F W_per^(F) - 2]/C
          pder[*,*,k] = ALOG10(ABS(ff*(pp[0]*w_perp - 2d0)/vteper[0]))
        END
        3     :  BEGIN
          ;;  dZ/dD = Z [F W_par^(F-1)]/B
          even_t      = ((pp[0] - 1d0) MOD 2) EQ 0
          IF (even_t[0]) THEN term0 = welpa2d^(pp[0] - 1d0) ELSE term0 = ABS(welpa2d)^(pp[0] - 1d0)
          pder[*,*,k] = ALOG10(ABS(ff*(pp[0]*term0)/vtepar[0]))
        END
        4     :  BEGIN
          ;;  dZ/dE = Z [F W_per^(F-1)]/C
          even_t      = ((pp[0] - 1d0) MOD 2) EQ 0
          IF (even_t[0]) THEN term0 = welpe2d^(pp[0] - 1d0) ELSE term0 = ABS(welpe2d)^(pp[0] - 1d0)
          pder[*,*,k] = ALOG10(ABS(ff*(pp[0]*term0)/vteper[0]))
        END
        5     :  BEGIN
          ;;  dZ/dF = Z { [3 DG(1 + 1/F)/F^2] - W_par^(F) Ln|W_par| - W_per^(F) Ln|W_per|}
          ndg         = 30000000L
