...
;                                                                   [04/23/2018   v1.1.2]
;             4)  Moved location of routine and ASCII files and now routine
;                   only reads in 20 values on either side of input argument to avoid
;                   slow down caused by reading large ASCII files
;                                                                   [04/24/2018   v1.1.3]
;
;   NOTES:      
...
;    LAST MODIFIED:  04/24/2018   v1.1.3
...
zz             = 1d0*z[0]
CASE 1 OF
  (zz[0] GT 0.0005 AND zz[0] LT 0.060) : BEGIN
    file_name      = 'digamma_numerical_estimates_0.0001-0.1_0.0001-steps.txt'
    mform          = '(f7.5,f30.20)'
    xr_dx_n        = [1d-4,0.0999d0,1d-4,999d0]                ;;  [X_min, X_max, dX, Nx]
  END
  (zz[0] GE 0.060 AND zz[0] LT 0.140) : BEGIN
    file_name      = 'digamma_numerical_estimates_0.05-0.15_0.0001-steps.txt'
    mform          = '(f7.5,f30.20)'
    xr_dx_n        = [0.0501d0,0.1499d0,1d-4,999d0]            ;;  [X_min, X_max, dX, Nx]
  END
  (zz[0] GE 0.140 AND zz[0] LT 10.05) : BEGIN
    file_name      = 'digamma_numerical_estimates_0.09-10.10_0.01-steps.txt'
    mform          = '(f5.2,f30.20)'
    xr_dx_n        = [9d-2,10.10d0,1d-2,1002d0]                ;;  [X_min, X_max, dX, Nx]
  END
  (zz[0] GE 10.05 AND zz[0] LT 20.00) : BEGIN
    file_name      = 'digamma_numerical_estimates_10.00-20.10_0.01-steps.txt'
    mform          = '(f5.2,f30.20)'
    xr_dx_n        = [ 1d1,20.10d0,1d-2,1011d0]                ;;  [X_min, X_max, dX, Nx]
  END
  (zz[0] GE 20.05 AND zz[0] LT 30.00) : BEGIN
    file_name      = 'digamma_numerical_estimates_20.00-30.10_0.01-steps.txt'
    mform          = '(f5.2,f30.20)'
    xr_dx_n        = [ 2d1,30.10d0,1d-2,1011d0]                ;;  [X_min, X_max, dX, Nx]
  END
  (zz[0] GE 30.05 AND zz[0] LT 40.85) : BEGIN
    file_name      = 'digamma_numerical_estimates_30.00-41.00_0.01-steps.txt'
    mform          = '(f5.2,f30.20)'
    xr_dx_n        = [ 3d1,40.99d0,1d-2,1100d0]                ;;  [X_min, X_max, dX, Nx]
  END
  (zz[0] GE 40.85 AND zz[0] LT 100.09) : BEGIN
    file_name      = 'digamma_numerical_estimates_40.0-101.0_0.05-steps.txt'
    mform          = '(f6.2,f30.20)'
    xr_dx_n        = [ 4d1,101d0,5d-2,1221d0]                ;;  [X_min, X_max, dX, Nx]
  END
  ELSE                                : BEGIN
  ...
  END
ENDCASE
;;  Define array of argument values from ASCII files
xx             = DINDGEN(LONG(xr_dx_n[3]))*(xr_dx_n[1] - xr_dx_n[0])/(xr_dx_n[3] - 1d0) + xr_dx_n[0]
;;----------------------------------------------------------------------------------------
;;  Define file location and name
;;----------------------------------------------------------------------------------------
file_dir       = tdir[0]+'LYNN_PRO'+slash[0]+'vdf_fitting_routines'+slash[0]
...
loc            = VALUE_LOCATE(xx,zz[0])
test           = (loc[0] LT 4) OR (loc[0] GE (nl[0] - 5L))
IF (test[0]) THEN BEGIN
  MESSAGE,'Input argument is out of bounds!',/INFORMATIONAL,/CONTINUE
  RETURN,0b
ENDIF
;;  Define array of indices to read in
low            = (loc[0] - 20L) > 0L
upp            = (loc[0] + 20L) < (nl[0] - 1L)
nin            = upp[0] - low[0] + 1L
skip           = (low[0] - 1L) > 0L                 ;;  # of lines to skip in file when reading
;;  Define empty arrays to fill
argz           = REPLICATE(d,nin[0])                ;;  argument of digamma function
psiz           = REPLICATE(d,nin[0])                ;;  result of digamma function
...
IF (err[0] NE 0) THEN BEGIN
  PRINT, -2, !ERROR_STATE.MSG   ;;  Prints an error message
  STOP                          ;;  debug
ENDIF
SKIP_LUN,gunit[0],skip[0],/LINES                    ;;  Skip lines in file
;FOR j=low[0], upp[0] DO BEGIN
;FOR j=0L, nl[0] - 1L DO BEGIN
FOR j=0L, nin[0] - 1L DO BEGIN
