...
;                                                                 [06/01/2011   v3.5.2]
;            38)  Changed manner in which frequency titles and plot labels were produced
;                                                                 [09/26/2011   v3.6.0]
;            39)  Allow for other spacecraft in SAT keyword       [11/03/2011   v3.6.1]
;
;   NOTES:      
...
;    LAST MODIFIED:  11/03/2011   v3.6.1
...
version  = vers2[0]+" and  plot_vector_mv_data.pro :  11/03/2011   v3.6.1"
;-----------------------------------------------------------------------------------------
...
CASE sn OF
  ...
  ELSE : BEGIN
    IF (STRLEN(sn[0]) EQ 3) THEN BEGIN
      fbna     = STRUPCASE(sn[0])+'_HTR'
      sat_titl = STRUPCASE(sn[0])+' '+xwin_inst
    ENDIF ELSE BEGIN
...
IF KEYWORD_SET(fcutoff) THEN BEGIN
  ...
  f_tags      = ['XMINOR','XLOG','XTICKS','XSTYLE','YSTYLE','XRANGE']
  IF (read_l) THEN x_min = 9L ELSE x_min = 4L
  freq_struct = CREATE_STRUCT(f_tags,x_min,read_l,5L,1,1,freqyra)
ENDIF ELSE BEGIN
