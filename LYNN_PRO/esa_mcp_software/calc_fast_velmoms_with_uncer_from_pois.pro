...
;               is_a_number.pro
;               is_a_3_vector.pro
;               lbw_fast_short_ninteg_3d_vdf.pro
;
;  REQUIRES:    
...
;                               [ Default = [0.,1.,0.] ]
;
;   CHANGED:  1)  Cleaned up and modified to reduce dependence on extra routines
;                   within lbw_fast_short_ninteg_3d_vdf.pro
;                                                                   [10/17/2024   v1.1.0]
;             2)  Updated expected structure format and tags to match
;                   lbw_fast_short_ninteg_3d_vdf.pro
;                                                                   [11/07/2024   v1.1.1]
;
...
;    LAST MODIFIED:  11/07/2024   v1.1.1
...
;;  Integrate f(v) ± ∂f(v)
;;----------------------------------------------------------------------------------------
fmdf_moms      = lbw_fast_short_ninteg_3d_vdf(vxyz,f_m_df,VLIM=vlim,NGRID=ngrid,SLICE2D=slice2d,$
...
fofv_moms      = lbw_fast_short_ninteg_3d_vdf(vxyz,f_of_v,VLIM=vlim,NGRID=ngrid,SLICE2D=slice2d,$
                                              VFRAME=vframe,ELECTRON=electron,PROTON=proton,    $
                                              ALPHA_P=alpha_p,MAG_DIR=mag_dir,VEC1=vec1,        $
                                              VEC2=vec2)
...
fpdf_moms      = lbw_fast_short_ninteg_3d_vdf(vxyz,f_p_df,VLIM=vlim,NGRID=ngrid,SLICE2D=slice2d,$
                                              VFRAME=vframe,ELECTRON=electron,PROTON=proton,    $
                                              ALPHA_P=alpha_p,MAG_DIR=mag_dir,VEC1=vec1,        $
                                              VEC2=vec2)
...
FOR k=0L, 2L DO BEGIN
  ...
  dumb_out.QFLUX_3VEC_XYZ[*,k] = [fmdf_moms.QFLUX_3VEC_XYZ[k],fofv_moms.QFLUX_3VEC_XYZ[k],fpdf_moms.QFLUX_3VEC_XYZ[k]]
  dumb_out.QFLUX_3VEC_FAC[*,k] = [fmdf_moms.QFLUX_3VEC_FAC[k],fofv_moms.QFLUX_3VEC_FAC[k],fpdf_moms.QFLUX_3VEC_FAC[k]]
  dumb_out.QFLUX_3VEC_NOR[*,k] = [fmdf_moms.QFLUX_3VEC_NOR[k],fofv_moms.QFLUX_3VEC_NOR[k],fpdf_moms.QFLUX_3VEC_NOR[k]]
ENDFOR
;;  rank-2 tensors
