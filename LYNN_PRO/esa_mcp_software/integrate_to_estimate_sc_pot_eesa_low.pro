...
;
;   CHANGED:  1)  Added some checking to make sure lower < upper values for either
;                   phi_sc or Ne
;                                                                   [10/29/2020   v1.0.1]
;
;   NOTES:      
...
;    LAST MODIFIED:  10/29/2020   v1.0.1
...
FOR j=0L, n_el[0] - 1L DO BEGIN
  ...
  ENDELSE
  ;;--------------------------------------------------------------------------------------
  ;;  Make sure lower < upper values
  ;;--------------------------------------------------------------------------------------
  test           = (best_phi[j,0L] GE best_phi[j,1L]) AND (TOTAL(FINITE(best_phi[j,*])) EQ 2)
  IF (test[0]) THEN best_phi[j,0L] = 9d-1*best_phi[j,1L]
  test           = (best_net[j,0L] GE best_net[j,1L]) AND (TOTAL(FINITE(best_net[j,*])) EQ 2)
  IF (test[0]) THEN best_net[j,0L] = 9d-1*best_net[j,1L]
