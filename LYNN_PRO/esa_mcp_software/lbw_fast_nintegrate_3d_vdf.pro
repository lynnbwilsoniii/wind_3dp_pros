...
;*****************************************************************************************
;
;  FUNCTION :   lbw_fast_nintegrate_3d_vdf.pro
...
;  EXAMPLES:    
;               [calling sequence]
;               vmom_str = lbw_fast_nintegrate_3d_vdf(vels,data [,VLIM=vlim] [,NGRID=ngrid] $
...
;
;   CHANGED:  1)  Routine now returns the dummy structure if the routine
;                   get_rotated_and_triangulated_vdf.pro returns a non-structure
;                                                                   [03/07/2024   v1.0.1]
;
;   NOTES:      0)  Adapted from lbw_nintegrate_3d_vdf.pro to increase computation
;                     speed when calling 1000s of times in a row
;               Velocity moments:
...
;    LAST MODIFIED:  03/07/2024   v1.0.1
;    MODIFIED BY: Lynn B. Wilson III
;
;*****************************************************************************************
;-

FUNCTION lbw_fast_nintegrate_3d_vdf,vels,data,VLIM=vlim,NGRID=ngrid,SLICE2D=slice2d,VFRAME=vframe,$
...
;;  Need to update ranges and increments
xran           = [MIN(x,/NAN),MAX(x,/NAN)]
yran           = [MIN(y,/NAN),MAX(y,/NAN)]
zran           = [MIN(z,/NAN),MAX(z,/NAN)]
dx             = (xran[1] - xran[0])/(nx[0] - 1L)
dy             = (yran[1] - yran[0])/(nx[0] - 1L)
dz             = (zran[1] - zran[0])/(nx[0] - 1L)
;;  Need to update h-factors for 3D Simpson's 1/3 Rule
hfac           = dx[0]*dy[0]*dz[0]/(3d0*3d0*3d0)
;;----------------------------------------------------------------------------------------
;;----------------------------------------------------------------------------------------
;;----------------------------------------------------------------------------------------
