...
;                                  [,P_LOG=p_log] [,C_LOG=c_log] [,TGRID=tgrid]        $
...
;    LAST MODIFIED:  05/31/2017   v1.8.2
...
PRO contour_3d_1plane,dat,vector1,vector2,VLIM=vlim,NGRID=ngrid,XNAME=xname,YNAME=yname, $
                          ...
                          P_LOG=p_log,C_LOG=c_log,TGRID=tgrid,SLICE2D=slice2d
...
IF (plog_on[0]) THEN levels = lg_levs ELSE levels = 1d1^lg_levs ;;  contour level values in phase space density [e.g., s^(+3) cm^(-3) km^(-3)]
...
IF KEYWORD_SET(sm_con) THEN BEGIN
  df2ds0  = SMOOTH(df2d,3L,/EDGE_TRUNCATE,/NAN)
ENDIF ELSE BEGIN
  df2ds0  = df2d
ENDELSE
;;  Smooth cuts if desired
IF KEYWORD_SET(sm_cuts) THEN BEGIN
  dfpars0        = SMOOTH(dfpara,ns[0],/EDGE_TRUNCATE,/NAN)
  dfpers0        = SMOOTH(dfperp,ns[0],/EDGE_TRUNCATE,/NAN)
  onec_par0      = SMOOTH(onec_para,ns[0],/EDGE_TRUNCATE,/NAN)
ENDIF ELSE BEGIN
  dfpars0        = dfpara
  dfpers0        = dfperp
  onec_par0      = onec_para
...
IF (test[0]) THEN BEGIN
  df2ds          = ALOG10(df2ds0)
  dfpars         = ALOG10(dfpars0)
  dfpers         = ALOG10(dfpers0)
  onec_par       = ALOG10(onec_par0)
  cut_yrange     = ALOG10(dfran)
ENDIF ELSE BEGIN
  df2ds          = df2ds0
  dfpars         = dfpars0
  dfpers         = dfpers0
...
IF (gdyl LT 20 AND gdyl GT 0) THEN BEGIN
  ...
  lim_ct0  = {XRANGE:xyran,     XSTYLE:1,XLOG:0,          XTITLE:cut_xttl,XMINOR:10,           $
              YRANGE:cut_yrange,YSTYLE:1,YLOG:ylog_cut[0],YTITLE:yttlct,  YMINOR:ymin_cut[0],   $
              POSITION:pos_0cut,TITLE:' ',NODATA:1,YTICKS:gdyl[0]-1L,                      $
              YTICKV:t_ytvs[goodyl],YTICKNAME:ytns[goodyl]}
;  lim_ct0  = {XRANGE:xyran, XSTYLE:1,XLOG:0,          XTITLE:cut_xttl,XMINOR:10,           $
...
PLOT,[0.0,1.0],[0.0,1.0],_EXTRA=lim_ct0
  ...
  OPLOT,vperp_cut*1d-3,dfpers,COLOR=blue[0],LINESTYLE=2
  ;;--------------------------------------------------------------------------------------
...
;test           = plog_on[0] AND ~clog_on[0]
;IF (test[0]) THEN BEGIN
;  ;;  Re-define parameters for output
;  df2ds          = 1d1^(df2ds)
;  dfpars         = 1d1^(dfpars)
;  dfpers         = 1d1^(dfpers)
;  onec_par       = 1d1^(onec_par)
;  cut_yrange     = 1d1^(dfran)
;  ;;  Change the limits structure
;ENDIF
