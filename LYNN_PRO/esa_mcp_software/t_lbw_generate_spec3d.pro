...
;  EXAMPLES:    
;               [calling sequence]
;               struc = sub_lbw_vframe_trans(dat [,FRCCNTS=frac_cnts] $
...
;    LAST MODIFIED:  02/27/2025   v1.0.0
;    MODIFIED BY: Lynn B. Wilson III
;
;*****************************************************************************************

FUNCTION sub_lbw_vframe_trans,dat,FRCCNTS=frac_cnts,RM_PHOTO_E=rmscpot,SC_FRAME=scframe,$
                                  UNITS=gunits,                                         $
                                  TTLE_EXT=ttle_ext,VFRAME=vframe,RMSC_SUFFX=rmsc_suffx    ;;  Outputs
...
;*****************************************************************************************
;
;  FUNCTION :   sub_lbw_calc_pads.pro
...
;  EXAMPLES:    
;               [calling sequence]
;               struc = sub_lbw_calc_pads(dat [,PA_SETT=pa_sett] [,VECTOR=vecdir] $
...
FUNCTION sub_lbw_calc_pads,dat,PA_SETT=pa_sett,VECTOR=vecdir,RM_PHOTO_E=rmscpot,$
                               GOOD_BINS=lbins
...
;*****************************************************************************************
;
;  FUNCTION :   sub_lbw_avg_pads.pro
;  PURPOSE  :   This is a subroutine used to average over the pitch-angle distributions
;                 (PADs) computed in sub_lbw_calc_pads.pro, where the averaging
...
;  EXAMPLES:    
;               [calling sequence]
;               padav = sub_lbw_avg_pads(data,pois,para)
;
;  KEYWORDS:    
;               NA
;
...
FUNCTION sub_lbw_avg_pads,data,pois,para
...
;*****************************************************************************************
;
;  FUNCTION :   sub_lbw_get_tplot_pard.pro
...
;  EXAMPLES:    
;               [calling sequence]
;               tpn_stuff = sub_lbw_get_tplot_pard(data,para [,SPEC=spec]                 $
...
FUNCTION sub_lbw_get_tplot_pard,data,para,SPEC=spec,UNITN=gunits,UNITT=punits,           $
                                PARTICLE=species,TTLE_PRE=yttl_pref,INST_PRE=inst_pref,  $
                                MID_TOUT=mid_out,SCPREF=scpref,FSUFFX=fsuffx,            $
                                TTLE_EXT=ttle_ext,DATA_MIN=data_min,DATA_MAX=data_max,   $
                                LIM_DATA=lim_data
...
;*****************************************************************************************
;
;  PROCEDURE:   t_lbw_generate_spec3d.pro
;  PURPOSE  :   This routine generates a [UNITS] vs time spectra as a stacked line plot
;                 from the distributions measured by (currently) THEMIS or Wind.  The
;                 data are regridded in both energy and time to make them uniform.
;
;  CALLED BY:   
;               NA
;
;  INCLUDES:
;               sub_lbw_vframe_trans.pro
;               sub_lbw_calc_pads.pro
;               sub_lbw_avg_pads.pro
;               sub_lbw_get_tplot_pard.pro
;
;  CALLS:
...
;
;  INPUT:
;               DATA        :  [N]-Element [structure] array associated with a known low
;                                energy ESA data structure from Wind/3DP
;                                [see get_?.pro, ? = el, elb, pl, or plb]
;
;  EXAMPLES:    
;               [calling sequence]
;               t_lbw_generate_spec3d,data [,LIM_STR=lim_esal] [,DATA_ERAN=leran]           $
;                              [,SC_FRAME=sc_frame] [,CUT_RAN=cut_ran] [,P_ANGLE=p_angle]   $
;                              [,SUNDIR=sundir] [,VECTOR=vec] [,UNITS=units]                $
;                              [,DATA_BINS=lbins] [,ONE_C=one_c] [,RM_PHOTO_E=rm_photo_e]   $
;                              [,NUM_PA=num_pa] [,MIDN=midn] [,TRANGE=trange] [,SPEC=spec]  $
;                              [,DATA_MIN=ldat_min] [,DATA_MAX=ldat_max]                    $
;                              [,XDAT_DATA=xdat_esal] [,YDAT_DATA=ydat_esal]                $
...
;               ***       DIRECT  INPUTS       ***
;               **********************************
;               LIM_STR     :  Scalar [structure] used for plotting the DATA input
...
;                                LIMITS keyword.
;               DATA_ERAN   :  [2]-Element [float/double] array defining the range of
;                                energies [eV] over which the data will be plotted
;                                [Default  :  [1,1500]]
;               SC_FRAME    :  If set, routine will fit the data in the spacecraft frame
;                                of reference rather than the eVDF's bulk flow frame
;                                [Default  :  FALSE]
...
;               DATA_BINS   :  [N]-Element [byte] array defining which the solid angle
;                                 bins should be plotted [i.e., BINS[good] = 1b]
;                                 and which bins should not be plotted [i.e.,
;                                 BINS[bad] = 0b].  One can also define bins as an array
;                                 of indices that define which solid angle bins to
;                                 plot.  If this is the case, then on output, BINS will
;                                 be redefined to an array of byte values specifying which
;                                 bins are TRUE or FALSE.
...
;                                [Default = FALSE]
;               DATA_MIN    :  Scalar [numeric] defining the maximum allowed value of the
;                                DATA data in UNITS to consider when computing plot
;                                ranges for TPLOT
;                                [Default = (smaller of 95% of 5th percentile and 1% of MEDIAN)]
;               DATA_MAX    :  Scalar [numeric] defining the maximum allowed value of the
;                                DATA data in UNITS to consider when computing plot
...
;               ***     ALTERED ON OUTPUT      ***
;               **********************************
;               XDAT_DATA   :  Set to a named variable to return the X data used in the
;                                spectra plot.
;                                [XDAT output from lbw_spec3d.pro]
;               YDAT_DATA   :  Set to a named variable to return the Y data used in the
;                                spectra plot.
...
PRO t_lbw_generate_spec3d,data,LIM_STR=lim_esal,DATA_ERAN=leran,SC_FRAME=sc_frame,          $
                               CUT_RAN=cut_ran,P_ANGLE=p_angle,SUNDIR=sundir,               $
                               VECTOR=vec,UNITS=units,DATA_BINS=lbins,ONE_C=one_c,          $
                               RM_PHOTO_E=rm_photo_e,NUM_PA=num_pa,MIDN=midn,               $
                               TRANGE=trange,SPEC=spec,DATA_MIN=ldat_min,DATA_MAX=ldat_max, $
                               XDAT_DATA=xdat_esal,YDAT_DATA=ydat_esal,                     $    ;;  Output
                               XDAT_ALL=xdat__all,YDAT_ALL=ydat__all,ONEC_ALL=onec_all,     $
                               OUT_STR=out_str
...
def_esaleran   = [1d0,1.5d3]            ;;  Default energy range [eV]
...
noinpt_msg     = 'User must supply an [N]-element array of velocity distribution functions as IDL structures...'
notstr_msg     = 'DATA must be an [N]-element array of IDL structures...'
notvdf_msg     = 'DATA must be velocity distribution functions as an IDL structures...'
diffsc_msg     = 'DATA must come from the same spacecraft...'
...
IF (N_PARAMS() LT 1) THEN BEGIN
  ...
  RETURN
ENDIF
IF ((SIZE(data,/TYPE) NE 8L OR N_ELEMENTS(data) LT 1)) THEN BEGIN
  MESSAGE,notstr_msg[0],/INFORMATIONAL,/CONTINUE
  RETURN
ENDIF
;;  Check to make sure distributions have the correct format
test_esa0      = test_wind_vs_themis_esa_struct(data[0],/NOM)
IF ((test_esa0.(0) + test_esa0.(1)) NE 1) THEN BEGIN
...
tra_min        = MIN(dat0.TIME,/NAN)
tra_max        = MAX(dat0.END_TIME,/NAN)
;;----------------------------------------------------------------------------------------
;;----------------------------------------------------------------------------------------
;;----------------------------------------------------------------------------------------
...
IF (test_esa0.(0)) THEN BEGIN
  ...
  IF (SIZE(strn0,/TYPE) NE 8) THEN BEGIN
...
;;----------------------------------------------------------------------------------------
oldrmscp       = rmscpot[0]
tdatl_sw       = sub_lbw_vframe_trans(dat0,FRCCNTS=frac_cnts[0],RM_PHOTO_E=rmscpot,      $
...
tdapl_sw       = sub_lbw_vframe_trans(dap0,FRCCNTS=frac_cnts[0],RM_PHOTO_E=oldrmscp,     $
                                           SC_FRAME=scframe[0],UNITS=gunits[0])
...
;;----------------------------------------------------------------------------------------
nn_all         = [nvdf0[0],n_en0[0],n_sa0[0]]
struc_l        = sub_lbw_calc_pads(tdatl_sw,PA_SETT=pa_sett,VECTOR=vecdir,RM_PHOTO_E=rmscpot,GOOD_BINS=lbins)
;;  Repeat for Poisson statistics
strup_l        = sub_lbw_calc_pads(tdapl_sw,PA_SETT=pa_sett,VECTOR=vecdir,RM_PHOTO_E=rmscpot,GOOD_BINS=lbins)
;;----------------------------------------------------------------------------------------
;;----------------------------------------------------------------------------------------
;;----------------------------------------------------------------------------------------
;;  Average over pitch-angles
...
strav_l0       = sub_lbw_avg_pads(struc_l,strup_l,pang_ra)
...
;;----------------------------------------------------------------------------------------
;;  Get TPLOT stuff
tpn_stuff0     = sub_lbw_get_tplot_pard(strav_l0,pang_ra,SPEC=spec_on,UNITN=gunits,        $
...
IF (SIZE(tpn_stuff0,/TYPE) NE 8) THEN STOP    ;;  Something is wrong --> Debug
;;----------------------------------------------------------------------------------------
;;  Send DATA PADs to TPLOT
;;----------------------------------------------------------------------------------------
struc          =   strav_l0
...
IF (test[0]) THEN eran_all = [MIN(eran_esl,/NAN),MAX(eran_esl,/NAN)]
...
all_dat_min    = ldatmin[0]
all_dat_max    = ldatmax[0]
tpn_stuffa     = sub_lbw_get_tplot_pard(struc_avg,pang_ra,SPEC=spec_on,UNITN=gunits,       $
                                        UNITT=punits,PARTICLE=species,TTLE_PRE=yttl_pref,  $
