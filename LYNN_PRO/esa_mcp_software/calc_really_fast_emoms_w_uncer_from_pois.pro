;*****************************************************************************************
;
;  FUNCTION :   sub_fast_short_nint_3d_evdf.pro
;  PURPOSE  :   Performs a numerical integration of a 3D electron velocity distribution
;                 function (VDF), using Simpson's 1/3 Rule, to find the velocity moments
;                 up to the heat flux vector.  The results are assumed to be in a
;                 cartesian coordinate basis.
;
;  CALLED BY:   
;               calc_really_fast_emoms_w_uncer_from_pois.pro
;
;  INCLUDES:
;               NA
;
...
;  EXAMPLES:    
;               [calling sequence]
;               sub_fast_short_nint_3d_evdf,vel1,dat1m,dat1f,dat1p [,VLIM=vlim]         $
;                        [,NGRID=ngrid] [,MAG_DIR=mag_dir] [,VEC1=vec1] [,VEC2=vec2]    $
...
FUNCTION sub_fast_short_nint_3d_evdf,vel1,dat1m,dat1f,dat1p,VLIM=vlim,NGRID=ngrid,      $
...
eV2J           = qq[0]                                    ;;  Energy associated with 1 eV of energy [ J --> J_1eV*energy{eV} = energy{J} ]
;;  Define default
vframe         = [0d0,0d0,0d0]                            ;;  Assumes km/s units on input
...
toler          = 1d-6
elec_on        = 1b
mass           = me_esa[0]                                ;;  Default to electron mass
...
vel_in0        = TRANSPOSE(vel_ltr + jitter)
;;  Define finite values (QGRID3.PRO cannot handle NaNs)
;test_vdf       = FINITE(dat1f)
test           = FINITE(vel_in0[0,*]) AND FINITE(vel_in0[1,*]) AND $
                 FINITE(vel_in0[2,*]) AND FINITE(dat1f)
good_in        = WHERE(test,gd_in,COMPLEMENT=bad_in,NCOMPLEMENT=bd_in)
IF (gd_in[0] EQ 0) THEN RETURN,dumb_out
vel_inp        = TEMPORARY(vel_in0[*,good_in])
datm           = TEMPORARY(dat1m[good_in])
data           = TEMPORARY(dat1f[good_in])
datp           = TEMPORARY(dat1p[good_in])
;;----------------------------------------------------------------------------------------
...
IF ((N_ELEMENTS(szfm) NE 3) OR (N_ELEMENTS(szfa) NE 3) OR (N_ELEMENTS(szfp) NE 3)) THEN BEGIN
...
test           = ((szfm[0] NE szfm[1]) OR (szfm[0] NE szfm[2])) OR ((szfa[0] NE szfa[1]) OR (szfa[0] NE szfa[2])) OR ((szfp[0] NE szfp[1]) OR (szfp[0] NE szfp[2]))
IF (test[0]) THEN BEGIN
...
;;  *** Clean up ***
;v2dgrid        = 0 & v3dx = 0 & v3dy = 0 & v3dz = 0 & fm = 0 & ff = 0 & fp = 0
...
;*****************************************************************************************
;
;  FUNCTION :   calc_really_fast_emoms_w_uncer_from_pois.pro
...
;                 ***  This is for electrons only!  ***
;
;  CALLED BY:   
;               NA
;
;  INCLUDES:
;               sub_fast_short_nint_3d_evdf.pro
;
;  CALLS:
;               sub_fast_short_nint_3d_evdf.pro
;
...
;  EXAMPLES:    
;               [calling sequence]
;               calc_really_fast_emoms_w_uncer_from_pois,vels,data,pois [,VLIM=vlim]    $
;                          [,NGRID=ngrid] [,MAG_DIR=mag_dir] [,VEC1=vec1] [,VEC2=vec2]
;
;  KEYWORDS:    
;               ***  INPUTS  ***
...
;                                                                   [MM/DD/YYYY   v1.0.0]
;
;   NOTES:      0)  Adapted from calc_really_fast_vmoms_w_uncer_from_pois.pro to increase
...
FUNCTION calc_really_fast_emoms_w_uncer_from_pois,vels,data,pois,VLIM=vlim,NGRID=ngrid,    $
                                                  MAG_DIR=mag_dir,VEC1=vec1,VEC2=vec2
...
;;  Integrate f(v) ± ∂f(v)
;;----------------------------------------------------------------------------------------
fopm_moms      = sub_fast_short_nint_3d_evdf(vxyz,f_m_df,f_of_v,f_p_df,VLIM=vlim,  $
                                             NGRID=ngrid,MAG_DIR=mag_dir,          $
                                             VEC1=vec1,VEC2=vec2)
