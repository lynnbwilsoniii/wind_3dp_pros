...
;
;   CHANGED:  1)  Fixed dummy output structure and reduced dependence on extra routines
;                   to help increase computational efficiency
;                                                                   [10/17/2024   v1.1.0]
;             2)  Fixed typos referencing severak undefined variables and fixed input
;                   formatting and removed NaNs
;                                                                   [11/07/2024   v1.1.1]
;
...
FUNCTION lbw_fast_short_ninteg_3d_vdf,vel1,dat1,VLIM=vlim,NGRID=ngrid,SLICE2D=slice2d,VFRAME=vframe,$
...
;v1             = REPLICATE(1d0,nnd[0]) # vec1            ;; [K,3]-Element array
;v2             = REPLICATE(1d0,nnd[0]) # vec2            ;; [K,3]-Element array
...
fxyz_3d        = QGRID3(vel_inp,data,tetra,DELTA=delv[0],DIMENSION=nm[0],MISSING=d,START=v_st[0])
;;  Check output
szf0           = SIZE(fxyz_3d,/DIMENSIONS)
test           = (N_ELEMENTS(szf0) NE 3)
IF (test[0]) THEN BEGIN
  MESSAGE,'0:  Failed tetrahedralization: returning without integration...',/INFORMATIONAL,/CONTINUE
  RETURN,dumb_out[0]
ENDIF
test           = (szf0[0] NE szf0[1]) OR (szf0[0] NE szf0[2])
IF (test[0]) THEN BEGIN
