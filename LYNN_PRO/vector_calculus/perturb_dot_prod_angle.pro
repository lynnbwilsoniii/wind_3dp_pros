...
;                                 [Default = FALSE]
;
;   CHANGED:  1)  Added keywords: USE_MED and USE_STAND
;                                                                   [07/12/2018   v1.0.1]
;
;   NOTES:      
...
FUNCTION perturb_dot_prod_angle,v1,v2,NAN=nan,RADIANS=radians,TH_0_90=th_0_90,        $
                                      ...
                                      USE_MED=use_med,USE_STAND=use_stand
...
;;  Check input
;;----------------------------------------------------------------------------------------
IF ((N_PARAMS() NE 2) OR (N_ELEMENTS(v1) EQ 0) OR (N_ELEMENTS(v2) EQ 0)) THEN BEGIN
  MESSAGE,noinput_mssg[0],/INFORMATIONAL,/CONTINUE
  RETURN,0b
ENDIF
;;  Check input format
IF ((is_a_3_vector(v1,/NOMSSG) EQ 0) OR (is_a_3_vector(v2,/NOMSSG) EQ 0)) THEN BEGIN
...
IF ((szd1[0] NE szd2[0]) AND ((szd1[0] NE 1) OR (szd2[0] NE 1))) THEN BEGIN
  ;;  Check to see if one of the inputs is a single vector
  IF ((szd1[0] NE szd2[0]) AND ((szd1[0] EQ 1) OR (szd2[0] EQ 1))) THEN BEGIN
...
IF ((TOTAL(IMAGINARY(fv1f)) EQ 0) AND (TOTAL(IMAGINARY(fv2f)) EQ 0)) THEN cmplx_on = 0b ELSE cmplx_on = 1b
;;----------------------------------------------------------------------------------------
;;  Check keywords
;;----------------------------------------------------------------------------------------
;;  Check RADIANS
IF ((N_ELEMENTS(radians) GE 1) AND KEYWORD_SET(radians)) THEN rad_on = 1b ELSE rad_on = 0b
;;  Check TH_0_90
IF ((N_ELEMENTS(th_0_90) GE 1) AND KEYWORD_SET(th_0_90)) THEN th90_on = 1b ELSE th90_on = 0b
;;  Check PLANE
IF (KEYWORD_SET(plane) AND (SIZE(plane,/TYPE) EQ 7)) THEN plane_on = 1b ELSE plane_on = 0b
;;  Check DELTA_V1
IF (is_a_3_vector(delta_v1,/NOMSSG)) THEN BEGIN
...
ENDELSE
;;  Check DELTA_V2
IF (is_a_3_vector(delta_v2,/NOMSSG)) THEN BEGIN
...
IF (med_on[0]) THEN BEGIN
  theta0         = MEDIAN(thetas,/DOUBLE,DIMENSION=3)          ;;  [N,3]-Element array
  gtheta         = MEDIAN(theta0,/DOUBLE,DIMENSION=2)          ;;  [N]-Element array
ENDIF ELSE BEGIN
...
ENDELSE
;;  Calculate uncertainties [radians]
IF (stand_on[0]) THEN BEGIN
  ;;  Use Standard deviation instead of range for uncertainties
  dth0           = STDDEV(thetas,NAN=nan,/DOUBLE,DIMENSION=3)  ;;  [N,3]-Element array
  dtheta         = STDDEV(dth0,NAN=nan,/DOUBLE,DIMENSION=2)    ;;  [N]-Element array
ENDIF ELSE BEGIN
