...
;    LAST MODIFIED:  04/07/2018   v1.0.1
  ...
  FOR k=0L, np[0] - 1L DO BEGIN
    IF (requested[k] NE 0) THEN BEGIN
      CASE k[0] OF
        ...
        1     :  BEGIN
          ;;  dZ/dB = Z [F W_par - 1]/B
          pder[*,*,k] = ff*(pp[0]*welpa2d - 1d0)/param[1]
        END
        2     :  BEGIN
          ;;  dZ/dC = Z [F W_per - 2]/C
          pder[*,*,k] = ff*(pp[0]*welpe2d - 2d0)/param[2]
        END
        3     :  BEGIN
          ;;  dZ/dD = Z [F W_par^(F-1)]/B
          IF (odd__p[0]) THEN term0 = welpa2d^(pp[0] - 1d0) ELSE term0 = ABS(welpa2d)^(pp[0] - 1d0)
          pder[*,*,k] = ff*(pp[0]*term0)/param[1]
        END
        4     :  BEGIN
          ;;  dZ/dE = Z [F W_per^(F-1)]/C
          IF (odd__p[0]) THEN term0 = welpe2d^(pp[0] - 1d0) ELSE term0 = ABS(welpe2d)^(pp[0] - 1d0)
