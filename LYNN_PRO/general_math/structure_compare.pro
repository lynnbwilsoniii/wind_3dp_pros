...
IF (exact[0]) THEN BEGIN
  ...
  IF (exact[0]) THEN BEGIN
    ...
    IF (exact[0]) THEN BEGIN
      ...
      match_tt       = 1b
      ;;  Increment:  VALUE_OUT
      ;;    [(MATCH_NT = TRUE && MATCH_TT = TRUE && MATCH_TG = TRUE) --> VALUE_OUT ≥ 6]
      value_out     += match_tt[0]
      count          = 0L
...
numovr_tt      = countt[0]
;;  Increment:  VALUE_OUT
value_out     += (countg[0] GT 0)
IF (countg[0] GT 0) THEN value_out += (countt[0] GT 0)
;;----------------------------------------------------------------------------------------
;;  Return to user
;;----------------------------------------------------------------------------------------
