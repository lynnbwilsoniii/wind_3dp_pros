...
;*****************************************************************************************
;
;  FUNCTION :   lbw__add.pro
;  PURPOSE  :   Finds the sum of two input scalars or arrays.  The output is given by:
;                    RESULT = VAL1 + VAL2
;
;  CALLED BY:   
;               NA
...
;  EXAMPLES:    
;               [calling sequence]
;               result = lbw__add(val1, val2 [,/NAN])
;
;  KEYWORDS:    
;               NAN   :  If set, routine will ignore NaNs during the summation
;                          [Default = FALSE]
;
;   CHANGED:  1)  NA
;                                                                   [MM/DD/YYYY   v1.0.0]
...
FUNCTION lbw__add,val1,val2,NAN=nan
...
d              = !VALUES.D_NAN
;;  Error messages
noinput_mssg   = 'User must supply two inputs from which the sum can be calculated...'
...
CASE N_ELEMENTS(szv1) OF
  1    : BEGIN
    ...
    IF (szv1[0] EQ 0) THEN summ = temp[0] ELSE summ = temp     ;;  check for scalar input
  END
  2    : BEGIN
    ;;  Both are 2D arrays
    array = [[[val1]],[[val2]]]
    summ  = TOTAL(array,3,NAN=nan)
  END
  ELSE : BEGIN
