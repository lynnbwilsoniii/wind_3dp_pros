...
;                                                                   [07/31/2017   v1.3.0]
;             6)  Added new folders to LYNN_PRO directory
;                                                                   [04/24/2018   v1.4.0]
;
;   NOTES:      
...
;    LAST MODIFIED:  04/24/2018   v1.4.0
...
sub_strs       = cc_line[0]+['Crib Sheets','Main Routine','Common Block Routines',    $
                 ...
                 'Vbulk: '+['Crib Sheets','Main Routine','Plotting Routines',         $
                 'Misc. Routines','Prompting Routines','Testing Routines',            $
                 'Documentation']                                                     ]
...
;;  Vbulk Change IDL Libraries extra
vdf_ex_out     = ex_pref_1[0]+ex_midn_0[0]+'//vdf_fitting_routines//.'
...
ex_out_str     = CREATE_STRUCT(jstr,stu_ex_out[0],thm_ex_out[0],             $
                               ...
                               str_ex_out[0],vsw_ex_out[0],vdf_ex_out[0])
;;----------------------------------------------------------------------------------------
...
;;----------------------------------------------------------------------------------------
sec_line       = BYTARR(nl[0],nsec[0])
sub_line       = BYTARR(nl[0],nsub[0])
sec_gind       = REPLICATE(-1,nsec[0])
sub_gind       = REPLICATE(-1,nsub[0])
FOR j=0L, nsec[0] - 1L DO BEGIN
  temp          = STRMATCH(flines,sec_strs[j],/FOLD_CASE)
  good          = WHERE(temp,gd)
  sec_line[*,j] = temp
  sec_gind[j]   = good[0]
ENDFOR
FOR j=0L, nsub[0] - 1L DO BEGIN
  temp          = STRMATCH(flines,sub_strs[j],/FOLD_CASE)
  good          = WHERE(temp,gd)
  sub_line[*,j] = temp
  sub_gind[j]   = good[0]
ENDFOR
