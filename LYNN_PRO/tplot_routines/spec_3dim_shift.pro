...
PRO spec_3dim_shift,dat,name,WSHIFT=wshift,DATN=datn,DATS=datns,NEW_NAME=new_name,$
                             RANGE_AVG=range_avg
...
mytags  = TAG_NAMES(d)
ntags   = N_TAGS(d)
gtplotv = WHERE(STRLOWCASE(mytags) EQ 'v1',gtpv)
...
;-----------------------------------------------------------------------------------------
newy = FLTARR(nd1,nd2,nd3)             ; => New data array
;-----------------------------------------------------------------------------------------
; => Determine order of energy bins (SST data reverses order from electrostatic detectors)
;-----------------------------------------------------------------------------------------
mxen = MAX(d.V1,/NAN,lxd1)             ; => Max energy which in structure
mnen = MIN(d.V1,/NAN,lnd1)             ; => Min " "
myne = (ARRAY_INDICES(d.V1,lnd1))[1]   ; => Element corresponding to mnen
myxe = (ARRAY_INDICES(d.V1,lxd1))[1]
...
;  yttl   = my_ytitle_tplot(name,TAG1=' Flux !C',TAG2=STRTRIM(nd3,2),TAG3=' shifted Energies')
  tname  = '_wsh'
  newmax = MAX(newy,/NAN)*1.1
  myd = CREATE_STRUCT('YTITLE',yttl,'X',d.X,'Y',newy,'V1',d.V1,'V2',d.V2,$
                      'YLOG',ylg,'LABELS',newlabs,'PANEL_SIZE',d.PANEL_SIZE)
...
;  yttl  = my_ytitle_tplot(name,TAG1=' Flux!C',TAG2='(#cm!U-2!Ns!U-1!Nsr!U-1!NeV!U-1!N)')
  tname  = '_sh'
ENDIF ELSE BEGIN
  ;---------------------------------------------------------------------------------------
...
;  yttl = my_ytitle_tplot(name,TAG1='N',TAG2='!C'+shname,LOCTAG=[1,0,2,3,4])
ENDELSE
...
IF (gyli GT 0) THEN BEGIN
  ...
  ymin1  = MIN(newy[gyind1[0,*],gyind1[1,*],gyind1[2,*]],/NAN)/1.1
  ymax1  = MAX(newy[gyind1[0,*],gyind1[1,*],gyind1[2,*]],/NAN)*1.1
