...
;             7)  Added keyword:  BNAME and now calls array_where.pro and
;                   transform_vframe_3d.pro                       [02/23/2012   v1.5.0]
;
;   NOTES:      
...
PRO eesa_pesa_low_to_tplot,DATE=date,TRANGE=trange,PLM=plm,PLBM=plbm,         $
                           ...
                           TO_SAVE=to_save,SUFFX=suffx,BNAME=bname
...
IF KEYWORD_SET(bname) THEN tbname = bname[0] ELSE tbname = 'wi_B3_GSE'
;-----------------------------------------------------------------------------------------
; => Check input
;-----------------------------------------------------------------------------------------
...
IF (pesa_str GT 0) THEN BEGIN
  pesa_low_moment_calibrate,DATE=date,TRANGE=tr3,COMPRESS=gcomp,MIDRA=midra,/NOLOAD,$
                            G_MAGF=g_magf,PLM=struct_p.(0),PLBM=struct_p.(1),BNAME=bname
ENDIF
;-----------------------------------------------------------------------------------------
...
IF (eesa_str GT 0) THEN BEGIN
  ...
  moments_array_3dp,PLM=ael,PLBM=aelb,MOMS=moms_sc,TO_TPLOT=to_tplot,$
                    SUFFX='el_sc'+suffx
  ;---------------------------------------------------------------------------------------
  ; => Calculate EESA Low DFs and convert into SWF
  ;---------------------------------------------------------------------------------------
  IF (SIZE(ael,/TYPE) EQ 8L) THEN BEGIN
    dumb    = conv_units(ael[0],'df')
    nel     = N_ELEMENTS(ael)
    sw_ael  = REPLICATE(dumb[0],nel)
    FOR j=0L, nel - 1L DO BEGIN
      dat       = ael[j]
      transform_vframe_3d,dat,/EASY_TRAN,/INTERP
      sw_ael[j] = dat[0]
    ENDFOR
  ENDIF ELSE BEGIN
    sw_ael  = 0
  ...
  ENDELSE
  ;---------------------------------------------------------------------------------------
  ; => Calculate EESA Low Burst DFs and convert into SWF
  ;---------------------------------------------------------------------------------------
  IF (SIZE(aelb,/TYPE) EQ 8L) THEN BEGIN
    dumb    = conv_units(aelb[0],'df')
    nelb    = N_ELEMENTS(aelb)
    sw_aelb = REPLICATE(dumb[0],nelb)
...
IF NOT KEYWORD_SET(to_tplot) THEN BEGIN
  ...
  overlap    = array_where(old_ind,new_ind,NCOMP1=bold1,NCOMP2=bnew2,/N_UNIQ)
