...
;                                                                   [04/21/2016   v2.2.1]
;             5)  Now makes sure to sort the two input times either from user or output
;                   from ctime.pro to avoid accidentally killing 100% of data if
;                   t[0] > t[1] and now calls t_set_trange.pro instead of
;                   set_tplot_times.pro
;                                                                   [05/28/2021   v2.2.2]
;
;   NOTES:      
...
;    LAST MODIFIED:  05/28/2021   v2.2.2
  ...
  CASE n[0] OF
    0    :  BEGIN
      ...
      CASE good[0] OF
        ...
        ELSE :  trange = [t1[0],t2[0]]    ;;  new range
      ENDCASE
    END
    1    :  BEGIN
      ;;----------------------------------------------------------------------------------
      ...
      ENDIF ELSE BEGIN
        ...
        IF (N_ELEMENTS(d1) NE 1) THEN BEGIN
          ...
          RETURN
        ENDIF
        trange = [gettime(d1[0]),gettime(d1[0]) + tr[1] - tr[0]]
      ENDELSE
    END
    2    :  BEGIN
      ;;----------------------------------------------------------------------------------
      ...
      ;;----------------------------------------------------------------------------------
      ;;  First sort
      IF (d1[0] LT d2[0]) THEN BEGIN
        ;;  Correct input --> Proceed
        trange = gettime([d1[0],d2[0]])
      ENDIF ELSE BEGIN
        ;;  Incorrect input --> Fix
        IF (d1[0] EQ d2[0]) THEN BEGIN
          ;;  Times are identical --> re-call routine
          kill_data_tr,d1[0],NAMES=names,MKILL=mkill,NKILL=nkill,TRA_KILLED=tra_killed,$
                             IND_2D=ind_2d,IND_3D=ind_3d
          ;;  Return to user
          RETURN
        ENDIF ELSE BEGIN
          ;;  Order is flipped --> correct
          trange = gettime([d2[0],d1[0]])
        ENDELSE
      ENDELSE
    END
    ELSE :  BEGIN
...
;    bad    = WHERE(dat0.X LE trange[1] AND dat0.X GE trange[0],bd)
    ...
    ENDCASE
    ;;------------------------------------------------------------------------------------
    ;;  Remove data between selected time stamps
    ;;------------------------------------------------------------------------------------
  ...
  IF (multi_kill[0] OR known_nkill[0]) THEN BEGIN
    ...
    ENDIF ELSE BEGIN
      ...
      WHILE (read_out[0] NE 'n' AND read_out[0] NE 'y' AND read_out[0] NE 'q') DO BEGIN
        ...
        PRINT, info1[0]
        PRINT, ""
        READ,read_out[0],PROMPT=mssg
        PRINT, ""
        IF (read_out[0] EQ 'debug') THEN STOP
      ENDWHILE
      ;;  make sure index has not exceeded 9999 and user wishes to continue
      true     = (kind[0] LT 9999L) AND (read_out[0] EQ 'y')
      IF (read_out[0] EQ 'q') THEN true = 0  ;; user wishes to quit
    ENDELSE
  ENDIF ELSE BEGIN
