...
ENDIF
;;  Check LOG___T
test           = (N_ELEMENTS(log___t) LT 1) OR (is_a_number(log___t) EQ 0)
...
temp_un2       = t_resample_tplot_struc(temp_un,new__t,/NO_EXTRAPOLATE)
temp_no2       = t_resample_tplot_struc(temp_no,new__t,/NO_EXTRAPOLATE)
temp_bo2       = t_resample_tplot_struc(temp_bo,new__t,/NO_EXTRAPOLATE)
...
IF (test_te[0]) THEN BEGIN
  ...
  IF (test_te[0]) THEN BEGIN
    ...
    temp_te2       = t_resample_tplot_struc(temp_te,new__t,/NO_EXTRAPOLATE)
    ;;  Define newly interpolated parameters
...
IF (test_ti[0]) THEN BEGIN
  ...
  IF (test_ti[0]) THEN BEGIN
    ;;  Good  -->  Interpolate parameters to new times
    temp_ti2       = t_resample_tplot_struc(temp_ti,new__t,/NO_EXTRAPOLATE)
    ;;  Define newly interpolated parameters
...
IF (test_th[0]) THEN BEGIN
  ...
  IF (test_th[0]) THEN BEGIN
    ;;  Good  -->  Interpolate parameters to new times
    temp_th2       = t_resample_tplot_struc(temp_th,new__t,/NO_EXTRAPOLATE)
    ;;  Define newly interpolated parameters
...
FOR j=0L, n_ct[0] - 1L DO BEGIN
  ...
  ENDIF ELSE BEGIN
    ;;  2D array
    test3v = is_a_3_vector(data,V_OUT=data2)
