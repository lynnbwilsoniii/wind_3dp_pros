...
;*****************************************************************************************
;
;  PROCEDURE:   fft_movie_oplot_freq.pro
;  PURPOSE  :   Overplots vertical lines on the power spectrum plot at user defined
;                 frequency values.
;
;  CALLED BY:   
;               fft_movie_plot_idlmpeg.pro
;
;  CALLS:
...
;
;  INPUT:
;               SEL         :  Scalar defining the start element of the current time-
;                                window being plotted
;               EEL         :  Scalar defining the end " "
;               PRAN        :  [2]-Element float array defining the power spectrum
...
;    LAST MODIFIED:  07/10/2012   v1.0.0
;    MODIFIED BY: Lynn B. Wilson III
;
;*****************************************************************************************
;-

PRO fft_movie_oplot_freq,sel,eel,pran,EX_FREQS=ex_freqs,EX_LABS=ex_labs,SCREEN=screen
...
IF KEYWORD_SET(ex_freqs) THEN BEGIN
  ...
  IF (szt) THEN BEGIN
    ...
    ENDIF ELSE BEGIN
      ...
      mxwdth = MAX(STRLEN(labels),/NAN)
    ENDELSE
    ;; define vertical position of labels
...
;*****************************************************************************************
;
;  PROCEDURE:   fft_movie_plot_idlmpeg.pro
;  PURPOSE  :   Plots the FFT snapshots of the input time series for fft_movie.pro using
;                 IDL's MPEG routines.
;
;  CALLED BY:   
;               fft_movie_plot.pro
...
PRO fft_movie_plot_idlmpeg,time,data,wlen,wshift,MOVIENAME=moviename,SCREEN=screen,   $
                   ...
                   QUALITY=quality,FFMPEG=ffmpeg,KEEP_SNAPS=keep_sn
...
IF (SIZE(wstruct,/TYPE) EQ 8L) THEN BEGIN
...
; => Open MPEG
;-----------------------------------------------------------------------------------------
IF KEYWORD_SET(quality) THEN qual = quality[0] ELSE qual = 25
...
;  stringtime    = STRING(startpoint[0]*timestep[0],FORMAT='(f6.2)')
  stringtime    = STRTRIM(STRING(startpoint[0]*timestep[0],FORMAT='(f15.4)'),2L)
  ...
  ; => Overplot frequencies if set
  ;---------------------------------------------------------------------------------------
  fft_movie_oplot_freq,dn_el1,up_el1,powra,EX_FREQS=ex_freqs,EX_LABS=ex_labs,SCREEN=screen
  ;---------------------------------------------------------------------------------------
  ; => Add frame to movie
  ;---------------------------------------------------------------------------------------
  MPEG_PUT,mpegID,IMAGE=TVRD(TRUE=1),FRAME=j[0],/ORDER
  ;---------------------------------------------------------------------------------------
  ; => Write to JPEG file
  ;---------------------------------------------------------------------------------------
  IF KEYWORD_SET(snapshots) THEN BEGIN
    IF (j EQ 1) THEN BEGIN
      ...
      ;-----------------------------------------------------------------------------------
      PLOT,nfreqbins,subpower[j,*],_EXTRA=fstr
      WRITE_JPEG,'blank_plot.jpg',TVRD(TRUE=1),TRUE=1,QUALITY=qual[0]
    ENDIF
    ; => Create JPEG files if necessary
    WRITE_JPEG,imgdir[0]+'/'+stringcount[0]+'.jpg',TVRD(TRUE=1),TRUE=1,QUALITY=qual[0]
  ENDIF
  ;;======================================================================================
  JUMP_SKIP:
  ;;======================================================================================
  startpoint  += wshift[0]
  endpoint    += wshift[0]
  IF (j MOD 100 EQ 0) THEN PRINT,'JPG File Count:',j
ENDFOR
;;  add extension to movie name
moviename = moviename[0]+m_ext[0]
PRINT,''
...
;*****************************************************************************************
;
;  PROCEDURE:   fft_movie_plot__ffmpeg.pro
;  PURPOSE  :   Plots the FFT snapshots of the input time series for fft_movie.pro using
;                 ImageMagick's ffmpeg routine.
;
;  CALLED BY:   
;               fft_movie_plot.pro
;
...
PRO fft_movie_plot__ffmpeg,time,data,wlen,wshift,MOVIENAME=moviename,SCREEN=screen,   $
                   FULLSERIES=fullseries,XSIZE=xsize,YSIZE=ysize,FFT_ARRAY=fft_array, $
                   NO_INTERP=no_interp,EX_FREQS=ex_freqs,EX_LABS=ex_labs,             $
                   FRANGE=frange,PRANGE=prange,WSTRUCT=wstruct,FSTRUCT=fstruct,       $
                   READ_WIN=read_win,FORCE_N=force_n,IMAGE_DIR=imgdir,                $
                   QUALITY=quality,FFMPEG=ffmpeg,FRAMERATE=framerate,                 $
                   KEEP_SNAPS=keep_sn
...
;  stringtime    = STRING(startpoint[0]*timestep[0],FORMAT='(f6.2)')
  ...
  ; => Write to PNG file
  ;---------------------------------------------------------------------------------------
  stringcount   = STRING(j[0],FORMAT='(i4.4)')
  ...
  startpoint  += wshift[0]
  endpoint    += wshift[0]
  IF (j MOD 100 EQ 0) THEN PRINT,'PNG File Count:',j
ENDFOR
;;  specify the frame rate
IF KEYWORD_SET(framerate) THEN frmra = LONG(framerate[0]) < 60L ELSE frmra = 15L
frm_str   = STRING(FORMAT='(I2.2)',frmra[0])
;;  add frame-rate and extension to movie name
moviename = moviename[0]+'_'+frm_str[0]+'fps'+m_ext[0]
PRINT,''
PRINT,'Encoding movie: '+moviename[0]
PRINT,''
;;  ffmpeg options
...
encode_command = pref_encode[0]+'/image_%04d.png -qscale 2 '+moviename[0]
...
;               QUALITY     :  Scalar value from 0-100 defining the MPEG and JPEG quality
;                                [see WRITE_JPEG.PRO and MPEG_OPEN.PRO documentation]
;               FFMPEG      :  If set, routine calls fft_movie_plot__ffmpeg.pro else
;                                calls fft_movie_plot_idlmpeg.pro
;               FRAMERATE   :  Scaler value defining the frame rate of output movie
;                                [Default = 25]
;
;   CHANGED:  1)  Now routine writes files as JPEG objects instead of PNG and
;                   added keywords:  SNAPSHOTS and QUALITY          [09/26/2011   v1.1.0]
;             2)  Added keyword:  FFMPEG and now calls fft_movie_plot__ffmpeg.pro and
;                   fft_movie_plot_idlmpeg.pro
...
;   CREATED:  06/15/2011
;   CREATED BY:  Lynn B. Wilson III
;    LAST MODIFIED:  07/10/2012   v2.0.0
;    MODIFIED BY: Lynn B. Wilson III
;
;*****************************************************************************************
;-
...
PRO fft_movie_plot,time,data,wlen,wshift,MOVIENAME=moviename,SCREEN=screen,           $
                   ...
                   FRANGE=frange,PRANGE=prange,WSTRUCT=wstruct,FSTRUCT=fstruct,       $
                   READ_WIN=read_win,FORCE_N=force_n,IMAGE_DIR=imgdir,                $
                   SNAPSHOTS=snapshots,QUALITY=quality,FFMPEG=ffmpeg,                 $
                   FRAMERATE=framerate
...
IF KEYWORD_SET(snapshots) THEN keep_sn = 1 ELSE keep_sn = 0
;-----------------------------------------------------------------------------------------
...
LOADCT,39,/SILENT
;;----------------------------------------------------------------------------------------
;; => Check which routine to call
;;----------------------------------------------------------------------------------------
IF KEYWORD_SET(ffmpeg) THEN BEGIN
  ;; Use ImageMagick's ffmpeg
  fft_movie_plot__ffmpeg,time,data,wlen,wshift,MOVIENAME=moviename,SCREEN=screen,     $
                   FULLSERIES=fullseries,XSIZE=xsize,YSIZE=ysize,FFT_ARRAY=fft_array, $
                   NO_INTERP=no_interp,EX_FREQS=ex_freqs,EX_LABS=ex_labs,             $
                   FRANGE=frange,PRANGE=prange,WSTRUCT=wstruct,FSTRUCT=fstruct,       $
                   READ_WIN=read_win,FORCE_N=force_n,IMAGE_DIR=imgdir,                $
                   QUALITY=quality,FRAMERATE=framerate,KEEP_SNAPS=keep_sn
ENDIF ELSE BEGIN
  ;; Use IDL's MPEG routines
  fft_movie_plot_idlmpeg,time,data,wlen,wshift,MOVIENAME=moviename,SCREEN=screen,     $
                   FULLSERIES=fullseries,XSIZE=xsize,YSIZE=ysize,FFT_ARRAY=fft_array, $
                   NO_INTERP=no_interp,EX_FREQS=ex_freqs,EX_LABS=ex_labs,             $
                   FRANGE=frange,PRANGE=prange,WSTRUCT=wstruct,FSTRUCT=fstruct,       $
                   READ_WIN=read_win,FORCE_N=force_n,IMAGE_DIR=imgdir,                $
                   QUALITY=quality,KEEP_SNAPS=keep_sn
ENDELSE
;-----------------------------------------------------------------------------------------
