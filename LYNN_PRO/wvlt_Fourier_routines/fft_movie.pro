...
;
;  REQUIRES:    
;               1)  ImageMagick's ffmpeg or IDL's MPEG routines
...
;                                by the program.  The movie extension (e.g. '.mov')
;                                will be added by this routine.
...
;               SNAPSHOTS   :  If set, routine will not delete image directory full of
...
;             3)  Now fft_movie_plot.pro checks whether the computer being used has
;                   ImageMagick installed, if so, then uses ffmpeg, else uses IDL's
;                   MPEG routines
;                                                                   [07/10/2012   v2.1.0]
;
;   NOTES:      
...
;               2)  I strongly recommend installing ImageMagick with ffmpeg prior to
;                     running this routine because ffmpeg is >10 x's faster than
;                     IDL's MPEG routines [and the quality is better].
;
;  REFERENCES:  
...
;    LAST MODIFIED:  07/10/2012   v2.1.0
...
PRO fft_movie,time,timeseries,fftlength,fftstep,MOVIENAME=moviename,       $
              ...
              FRAMERATE=framerate,KEEP_SNAPS=keep_sn
...
IF ~KEYWORD_SET(moviename) THEN BEGIN
  ...
  READ,mname,PROMPT='('+m_ext[0]+' extension will be added): '
ENDIF ELSE mname = moviename
;;mname = mname[0]+m_ext[0]
;;----------------------------------------------------------------------------------------
;; => Check input
;;----------------------------------------------------------------------------------------
times         = REFORM(time)
