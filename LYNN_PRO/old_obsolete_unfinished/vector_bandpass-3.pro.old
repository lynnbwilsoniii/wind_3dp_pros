...
;                                                                   [12/04/2015   v3.0.0]
;             5)  Fixed a potential issue if user defined a frequency range with no
;                   excluded bins, the routine could break but now will not
;                                                                   [04/06/2017   v3.0.1]
;
;   NOTES:      
...
;    LAST MODIFIED:  04/06/2017   v3.0.1
...
lfc1           = 1d0*lf[0]
hfc1           = 1d0*hf[0]
good_low       = (ABS(fft_freq) LE lfc1[0]) AND (ABS(fft_freq) GT 0d0)
good_mid       = (ABS(fft_freq) GT lfc1[0]) AND (ABS(fft_freq) LE hfc1[0])
good_upp       = (ABS(fft_freq) GT hfc1[0])
lowf1          = WHERE(good_low,lf1,COMPLEMENT=other_mh,NCOMPLEMENT=bd_oth_mh)
midf1          = WHERE(good_mid,mf1,COMPLEMENT=other_lh,NCOMPLEMENT=bd_oth_lh)
highf1         = WHERE(good_upp,hf1,COMPLEMENT=other_lm,NCOMPLEMENT=bd_oth_lm)
