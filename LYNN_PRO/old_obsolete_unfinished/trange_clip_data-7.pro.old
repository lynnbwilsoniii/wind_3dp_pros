...
;    LAST MODIFIED:  06/15/2016   v1.3.0
...
IF (test[0]) THEN BEGIN
  ...
  IF (v1_v2_on[0]) THEN BEGIN
    ...
    struc          = {X:xx,Y:yy,V1:v1out,V2:v2out,TSHIFT:tshift[0]}
;    struc          = {X:xx,Y:yy,V1:v1out,V2:v2out}
  ENDIF ELSE BEGIN
    ;;  V is present
    struc          = {X:xx,Y:yy,V:vvout,TSHIFT:tshift[0]}
;    struc          = {X:xx,Y:yy,V:vvout}
  ENDELSE
ENDIF ELSE BEGIN
  ;;  Only X and Y tags are present or correctly set
  struc          = {X:xx,Y:yy,TSHIFT:tshift[0]}
;  struc          = {X:xx,Y:yy}
ENDELSE
;;  Check for DY
IF (test_uy[0]) THEN str_element,struc,'DY',dy,/ADD_REPLACE  ;;  Add DY (if present)
