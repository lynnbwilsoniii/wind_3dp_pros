...
;    LAST MODIFIED:  06/15/2016   v1.4.0
...
CASE sznd[0] OF
  1    :  BEGIN
    yy = dats[sp]          ;;  [N]-Element array
    IF (test_uy[0]) THEN dy = uny[sp]
  END
  2    :  BEGIN
    yy = dats[sp,*]        ;;  [N,M]-Element array
    IF (test_uy[0]) THEN dy = uny[sp,*]
  END
  3    :  BEGIN
    yy = dats[sp,*,*]      ;;  [N,M,L]-Element array
    IF (test_uy[0]) THEN dy = uny[sp,*,*]
  END
...
;  IF (gd_int[0] GT 1) THEN FOR ii=1L, n__int[0] - 1L DO ind = [ind,LINDGEN(ni[ii]) + good_st[ii]]
  ...
  CASE sznd[0] OF
    1    :  BEGIN
      ...
      IF (test_uy[0]) THEN dy = TEMPORARY(dy[ind])
    END
    2    :  BEGIN
...
CASE sznd[0] OF
  1    :  BEGIN
    yout = REPLICATE(d,nx)
    IF (test_uy[0]) THEN dyout = REPLICATE(d,nx)
  END
  2    :  BEGIN
    yout = REPLICATE(d,nx,szdd[1])                ;;  [N,M]-Element array
    vout = REPLICATE(d,nx,szdd[1])
    IF (test_uy[0]) THEN dyout = REPLICATE(d,nx,szdd[1])
  END
  3    :  BEGIN
    ...
    IF (test_uy[0]) THEN dyout = REPLICATE(d,nx,szdd[1],szdd[2])
  END
ENDCASE
;;----------------------------------------------------------------------------------------
;;  Interpolate
...
IF (yes_overlap[0]) THEN BEGIN
  CASE sznd[0] OF
    1    :  BEGIN
      ...
      IF (test_uy[0]) THEN dyout = INTERPOL(dy,xx_in,xout,_EXTRA=int_str)
    END
    2    :  BEGIN
      ;;  2D array --> may have V tag
      FOR d2=0L, szdd[1] - 1L DO BEGIN
        yout[*,d2] = INTERPOL(yy[*,d2],xx_in,xout,_EXTRA=int_str)
        IF (test_uy[0]) THEN dyout[*,d2] = INTERPOL(dy[*,d2],xx_in,xout,_EXTRA=int_str)
        IF (test_vv[0]) THEN BEGIN
          IF (test_vv_2d[0]) THEN BEGIN
    ...
    3    :  BEGIN
      FOR d2=0L, szdd[1] - 1L DO BEGIN
        FOR d3=0L, szdd[2] - 1L DO BEGIN
          ...
          IF (test_uy[0]) THEN dyout[*,d2,d3] = INTERPOL(dy[*,d2,d3],xx_in,xout,_EXTRA=int_str)
