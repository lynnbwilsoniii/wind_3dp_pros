...
;;  Check inputs
;;----------------------------------------------------------------------------------------
test           = (is_a_3_vector(vels,V_OUT=swfv,/NOMSSG) EQ 0) OR                          $
                 (is_a_number(data,/NOMSSG) EQ 0) OR (is_a_number(rotmxy,/NOMSSG) EQ 0) OR $
                 (is_a_number(rotmzy,/NOMSSG) EQ 0)
...
;;----------------------------------------------------------------------------------------
IF (clog_on[0]) THEN testf = FINITE(dat_1d) ELSE testf = FINITE(dat_1d) AND (dat_1d GE 0.)
...
IF (cnt GT 0L) THEN BEGIN
  ...
  ;;  Add small variation about actual values to avoid colinear points error
