...
; => Check input
;-----------------------------------------------------------------------------------------
ni         = REFORM(rho)          ; => [2]-Element array
bo         = REFORM(mag)          ; => [3,2]-Element array
vo         = REFORM(vsw)          ; => [3,2]-Element array
te         = REFORM(tot)          ; => [2]-Element array
...
;-----------------------------------------------------------------------------------------
n     = N_ELEMENTS(vshn)
sum   = DBLARR(n)      ; =>  sum_{j=1}^{M} (Y_{j}(x_{i},a)/std[j]_{i})^{2}
