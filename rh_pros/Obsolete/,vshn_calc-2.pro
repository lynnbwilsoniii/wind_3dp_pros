...
; => LBW III 09/09/2011
test  = ((szv[1] NE 3) OR (szv[2] NE 2)) OR (szv[0] NE szr[0]) OR (szn[1] NE 3)
; => LBW III 09/07/2011
...
;-----------------------------------------------------------------------------------------
vshn  = DBLARR(szv[0],szn[0])        ; => [N,M]-Element array
term0 = ratio[*,0] # n1[*,0]
term1 = ratio[*,1] # n1[*,1]
term2 = ratio[*,2] # n1[*,2]
vshn  = term0 + term1 + term2
;FOR j=0L, szv[0] - 1L DO BEGIN
;  vshn[j,*] = ratio[j,0]*n1[*,0] + ratio[j,1]*n1[*,1] + ratio[j,2]*n1[*,2]
;ENDFOR
