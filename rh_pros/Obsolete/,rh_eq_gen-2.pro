...
rhou       = REFORM(rhoud[*,0])   ; => [N]-Element
rhod       = REFORM(rhoud[*,1])   ; => [N]-Element
; => Define thermal pressure [Pa]
ptup       = (REFORM(rho[*,0])*1d6)*te  ; => Pa {= N m^(-2) = J m^(-3)} [N,2]-Element
ptdn       = (REFORM(rho[*,1])*1d6)*te  ; => Pa {= N m^(-2) = J m^(-3)} [N,2]-Element
...
; => Calc. the squares
delvvu2    = my_dot_prod(delvvu,delvvu,/NOM)/2d0
delvvd2    = my_dot_prod(delvvd,delvvd,/NOM)/2d0
...
nor2[*,1]  = nor[1]
nor2[*,2]  = nor[2]
vn[*,0,0]  = vec_norm(delvvu,nor2)  & vn[*,1,0] = vn[*,0,0] & vn[*,2,0] = vn[*,0,0]
vn[*,0,1]  = vec_norm(delvvd,nor2)  & vn[*,1,1] = vn[*,0,1] & vn[*,2,1] = vn[*,0,1]
...
vt[*,*,0]  = vec_trans(delvvu,nor2)
vt[*,*,1]  = vec_trans(delvvd,nor2)
...
CASE eqn[0] OF
  1  :  BEGIN
    ...
    eq0[*,0] = rhod*delvsd - rhou*delvsu
    eq0[*,1] = rhod*delvsd - rhou*delvsu
    eq0[*,2] = rhod*delvsd - rhou*delvsu
  END
  2  :  BEGIN
  ...
  5  :  BEGIN
    ...
    btm      = TOTAL(bt^2,2L,/NAN)/(2d0*muo[0])               ; => [N,2]-Element array
    dynu     = rhou*delvsu^2                                  ; => [N]-Element array
    dynd     = rhod*delvsd^2                                  ; => [N]-Element array
    eup      = ptup + btm[*,0] + dynu
    edn      = ptdn + btm[*,1] + dynd
    eq0      = REPLICATE(d,n,3L)
    eq0[*,0] = (edn - eup)
  ...
  6  :  BEGIN
    ...
    facu     = rhou*delvsu
    facd     = rhod*delvsd
    bmgu     = TOTAL(bou^2,2L,/NAN)/(rhou*muo[0])
    bmgd     = TOTAL(bod^2,2L,/NAN)/(rhod*muo[0])
    entu     = gfactor[0]*ptup/rhou
    entd     = gfactor[0]*ptdn/rhod
    eup      = facu*(delvvu2 + entu + bmgu)
    edn      = facd*(delvvd2 + entd + bmgd)
