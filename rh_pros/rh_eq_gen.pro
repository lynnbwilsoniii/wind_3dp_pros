...
;  EXAMPLES:    
;               [calling sequence]
;               rheq = rh_eq_gen(rho,vsw,mag,tot,nor0,vshn0 [,EQNUM=eqnum] [,POLYT=polyt])
;
;  KEYWORDS:    
;               EQNUM  :  Scalar [numeric] defining the equation number to generate
;                           1 = mass flux
;                           2 = normal B-field
...
;                                                                   [09/07/2011   v1.1.0]
;             2)  Changed/Checked calculations
;                                                                   [09/09/2011   v1.2.0]
;             3)  Vectorized routine and now calls rh_resize.pro and vec_cross.pro
;                                                                   [09/10/2011   v1.3.0]
;             4)  Updated man page now that subroutines are standalone
;                                                                   [07/11/2012   v1.4.0]
;             5)  Cleaned up, updated Man. page, and optimized
;                                                                   [08/28/2020   v1.4.1]
;
;   NOTES:      
...
dumb3d         = REPLICATE(d,nd[0],m_n[0],3L)
...
vou            = REFORM(vo[*,*,0],nd[0],3)  ;;  Upstream     [N,3]-Element array
vod            = REFORM(vo[*,*,1],nd[0],3)  ;;  Downstream   [N,3]-Element array
bou            = REFORM(bo[*,*,0],nd[0],3)  ;;  Upstream     [N,3]-Element array
bod            = REFORM(bo[*,*,1],nd[0],3)  ;;  Downstream   [N,3]-Element array
;;----------------------------------------------------------------------------------------
;;  Calculate (V . n - Vs)
;;----------------------------------------------------------------------------------------
delv_sc    = del_vsn(vo,nor,vshn,VEC=0)            ;;  [N,M,2]-Element array
delvs_up   = REFORM(delv_sc[*,*,0],nd[0],m_n[0])   ;;  [N,M]-Element array
delvs_dn   = REFORM(delv_sc[*,*,1],nd[0],m_n[0])   ;;  [N,M]-Element array
;;----------------------------------------------------------------------------------------
...
;;----------------------------------------------------------------------------------------
n_x_Vtu        = dumb3d                     ;;  [N,M,3]-Element array
n_x_Vtd        = dumb3d
n_x_Btu        = dumb3d
n_x_Btd        = dumb3d
j              = 0L
true           = 1
WHILE(true) DO BEGIN
  vt0u           = REFORM(vtu[j,*,*],m_n[0],3L)
  vt0d           = REFORM(vtd[j,*,*],m_n[0],3L)
  n_x_Vtu[j,*,*] = vec_cross(nor,vt0u)      ;;  [M,3]-Element array
  n_x_Vtd[j,*,*] = vec_cross(nor,vt0d)      ;;  [M,3]-Element array
  bt0u           = REFORM(btu[j,*,*],m_n[0],3L)
  bt0d           = REFORM(btd[j,*,*],m_n[0],3L)
...
delvv_up       = REFORM(delv_vec[*,*,*,0],nd[0],m_n[0],3L)  ;;  [N,M,3]-Element array
delvv_dn       = REFORM(delv_vec[*,*,*,1],nd[0],m_n[0],3L)  ;;  [N,M,3]-Element array
;;  Calculate { V - (Vs . n) n } . B
term0          = delvv_up[*,*,0]*(bou[*,0] # ones)
term1          = delvv_up[*,*,1]*(bou[*,1] # ones)
term2          = delvv_up[*,*,2]*(bou[*,2] # ones)
dv_dot_b_u     = term0 + term1 + term2                      ;;  [N,M]-Element array
term0          = delvv_dn[*,*,0]*(bod[*,0] # ones)
term1          = delvv_dn[*,*,1]*(bod[*,1] # ones)
term2          = delvv_dn[*,*,2]*(bod[*,2] # ones)
